...
Ce guide explique comment implémenter `ListView` et les différentes `Adapter` classes dans Xamarin. Android. Il montre également comment personnaliser l’apparence d’un `ListView` et décrit l’importance de la réutilisation de ligne pour réduire la consommation de mémoire. Il y a également une explication de l’impact et de l’utilisation du cycle de vie de l’activité `ListView` `Adapter` . Si vous travaillez sur des applications multiplateformes avec Xamarin. iOS, le `ListView` contrôle est structurellement similaire à IOS `UITableView` (et Android `Adapter` est semblable à `UITableViewSource` ).

Tout d’abord, un bref didacticiel présente le `ListView` avec un exemple de code de base. Vous trouverez ensuite des liens vers des rubriques plus avancées qui vous aideront à utiliser `ListView` dans des applications réelles.

> [!NOTE]
> Le `RecyclerView` widget est une version plus avancée et flexible de `ListView` . Étant donné que `RecyclerView` est conçu pour être le successeur de `ListView` (et `GridView` ), nous vous recommandons d’utiliser `RecyclerView` plutôt que `ListView` pour le nouveau développement d’applications. Pour plus d’informations, consultez [RecyclerView](~/android/user-interface/layouts/recycler-view/index.md).

## <a name="listview-tutorial"></a>Didacticiel ListView

[`ListView`](xref:Android.Widget.ListView) est un [`ViewGroup`](xref:Android.Views.ViewGroup)
qui crée une liste d’éléments déroulants. Les éléments de liste sont automatiquement insérés dans la liste à l’aide d’un [`IListAdapter`](xref:Android.Widget.IListAdapter) .
...
le constructeur prend l’application [`Context`](xref:Android.Content.Context) , la description de la disposition pour chaque élément de liste (créé à l’étape précédente) et un `T[]` ou [`Java.Util.IList<T>`](xref:Java.Util.IList)
...
Par exemple, essayez d’utiliser à la `Android.Resource.Layout.SimpleListItem1` place de `Resource.Layout.list_item` .
...
Exécutez l'application. Vous pouvez faire défiler la liste ou taper pour la filtrer, puis cliquer sur un élément pour afficher un message. Un résultat semblable à celui-ci doit s’afficher :
...
sélection. La meilleure pratique consiste à référencer un tableau de chaînes défini par une ressource externe, par exemple avec une `string-array` ressource dans le fichier de **ressources/valeurs/Strings.xml** du projet. Par exemple :
...
Pour utiliser ces chaînes de ressources pour le [`ArrayAdapter`](xref:Android.Widget.ArrayAdapter`1) , remplacez l’original [`ListAdapter`](xref:Android.App.ListActivity.ListAdapter)
...
Les rubriques restantes (liées ci-dessous) présentent une vue d’ensemble complète de l’utilisation de la `ListView` classe et des différents types de types d’adaptateurs que vous pouvez utiliser avec celle-ci. La structure est comme suit :

- **Apparence visuelle** &ndash; Les parties du `ListView` contrôle et leur fonctionnement.

- **Classes** &ndash; Vue d’ensemble des classes utilisées pour afficher un   `ListView` .

- **Affichage des données dans un ListView** &ndash; Comment afficher une simple liste de données ; comment implémenter `ListView's` des fonctionnalités d’utilisation, comment utiliser différentes dispositions de ligne intégrées et comment les adaptateurs enregistrent la mémoire en réutilisant les vues de lignes.

- **Apparence personnalisée** &ndash; Modification du style de `ListView` avec des dispositions personnalisées, des polices et des couleurs.

- **Utilisation de SQLite** &ndash; Comment afficher les données d’une base de données SQLite avec un `CursorAdapter` .

- **Cycle de vie** &ndash; des activités Considérations relatives à la conception lors de `ListView` l’implémentation d’activités, y compris où dans le cycle de vie vous devez remplir vos données et quand libérer des ressources.

La discussion (divisée en six parties) commence par une vue d’ensemble de la `ListView` classe elle-même avant d’introduire des exemples plus complexes de l’utilisation de la classe.

- [Composants et fonctionnement de ListView](~/android/user-interface/layouts/list-view/parts-and-functionality.md)
- [Remplissage d’un ListView avec des données](~/android/user-interface/layouts/list-view/populating.md)
- [Personnalisation de l’apparence d’un ListView](~/android/user-interface/layouts/list-view/customizing-appearance.md)
...
- [ListView et cycle de vie des activités](~/android/user-interface/layouts/list-view/activity-lifecycle.md)

## <a name="summary"></a>Résumé

Cet ensemble de rubriques a présenté `ListView` et fourni des exemples d’utilisation des fonctionnalités intégrées de `ListActivity` . Il a abordé les implémentations personnalisées de `ListView` qui étaient autorisées pour les dispositions colorées et l’utilisation d’une base de données SQLite, et elle a rapidement touché la pertinence du cycle de vie de l’activité sur votre `ListView` implémentation.

## <a name="related-links"></a>Liens associés

- [AccessoryViews (exemple)](/samples/xamarin/monodroid-samples/accessoryviews)
- [BasicTableAndroid (exemple)](/samples/xamarin/monodroid-samples/basictableandroid)
- [BasicTableAdapter (exemple)](/samples/xamarin/monodroid-samples/basictableadapter)
...
- [FastScroll (exemple)](/samples/xamarin/monodroid-samples/fastscroll)
- [SectionIndex (exemple)](/samples/xamarin/monodroid-samples/sectionindex)
