...
1. **Lecture audio avec MediaPlayer** &ndash; Utilisation de la classe intégrée  `MediaPlayer` pour lire l’audio, y compris les fichiers audio locaux et les fichiers audio diffusés avec la `AudioTrack` classe.

2. **Enregistrement audio** &ndash; Utilisation de la classe intégrée `MediaRecorder` pour enregistrer l’audio.
...
4. **Utilisation de l’audio** &ndash; de bas niveau Lecture audio à l’aide de la `AudioTrack` classe en écrivant directement dans des mémoires tampons. Enregistrement audio à l’aide de la `AudioRecord` classe et lecture directe à partir de mémoires tampons.
...
![Section autorisations requises du manifeste Android avec enregistrement \_ audio activé](android-audio-images/image01.png)
...
La lecture audio avec `MediaPlayer` requiert la séquence suivante :
...
La classe MediaRecorder pour l' `MediaPlayer` enregistrement audio dans Android est la classe [MediaRecorder](xref:Android.Media.MediaRecorder) . Comme le `MediaPlayer` , il est sensible à l’État et passe par plusieurs États pour atteindre le point où il peut commencer à enregistrer. Pour pouvoir enregistrer l’audio, l' `RECORD_AUDIO` autorisation doit être définie. Pour obtenir des instructions sur la façon de définir des autorisations d’application, consultez [utilisation de AndroidManifest.xml](~/android/platform/android-manifest.md).

### <a name="initializing-and-recording"></a>Initialisation et enregistrement

L’enregistrement de l’audio avec le `MediaRecorder` requiert les étapes suivantes :
...
Une fois que l' `MediaRecorder` a été arrêté, appelez la méthode de [réinitialisation](xref:Android.Media.MediaRecorder.Reset) pour rétablir l’état d’inactivité :

```csharp
recorder.Reset();
```

Lorsque `MediaRecorder` n’est plus nécessaire, ses ressources doivent être libérées en appelant la méthode [Release](xref:Android.Media.MediaRecorder.Release) :
...
Le focus audio est une idée similaire et empêche la lecture ou l’enregistrement audio de plusieurs applications en même temps. Il est plus compliqué que le focus clavier, car il est volontaire que &ndash; l’application peut ignorer ce fait qu’elle n’a pas actuellement le focus audio et qu’elle est exécutée, qu’il &ndash; y ait différents types de focus audio qui peuvent être demandés. Par exemple, si le demandeur est supposé uniquement lire de l’audio pendant un très bref laps de temps, il peut demander un focus temporaire.
...
L’inscription du `FocusChangeListener` rappel à partir de `IOnAudioChangeListener` est une partie importante de l’obtention et de la libération du focus audio. Cela est dû au fait que l’octroi du focus audio peut être différé jusqu’à une date ultérieure. Par exemple, une application peut demander à écouter de la musique pendant un appel téléphonique en cours. Le focus audio n’est pas accordé tant que l’appel téléphonique n’est pas terminé.

Pour cette raison, l’objet de rappel est passé en tant que paramètre dans la `GetAudioFocus` méthode de `AudioManager` , et c’est cet appel qui inscrit le rappel. Si le focus audio est initialement refusé, mais accordé ultérieurement, l’application est informée en appelant `OnAudioFocusChange` sur le rappel. La même méthode est utilisée pour indiquer à l’application que le focus audio est retiré.

Lorsque l’application a terminé d’utiliser les ressources audio, elle appelle la `AbandonFocus` méthode du `AudioManager` et transmet à nouveau le rappel. Cela annule l’inscription du rappel et libère les ressources audio, afin que d’autres applications puissent obtenir le focus audio.
...
Les API audio de bas niveau offrent un contrôle accru sur la lecture et l’enregistrement audio, car elles interagissent directement avec les mémoires tampons au lieu d’utiliser des URI de fichier. Dans certains cas, cette approche est préférable. Il s’agit entre autres des scénarios suivants :
...
4. Format audio &ndash; 8 bits ou 16 bits.

5. Taille de la mémoire tampon &ndash; en octets.

6. Streaming en mode de mémoire tampon &ndash; ou statique.

Après la construction, la méthode [Play](xref:Android.Media.AudioTrack.Play) de est appelée afin de la `AudioTrack` configurer pour démarrer la lecture. L’écriture de la mémoire tampon audio sur le `AudioTrack` démarre la lecture :
...
La classe [AudioRecord](xref:Android.Media.AudioRecord) est l’équivalent du `AudioTrack` côté enregistrement. `AudioTrack`À l’instar de, il utilise des mémoires tampons directement, à la place des fichiers et des URI. L' `RECORD_AUDIO` autorisation doit être définie dans le manifeste.

#### <a name="initializing-and-recording"></a>Initialisation et enregistrement

La première étape consiste à construire un nouvel objet [AudioRecord](xref:Android.Media.AudioRecord) . La liste d’arguments passée au [constructeur](xref:Android.Media.AudioRecord) fournit toutes les informations requises pour l’enregistrement. Contrairement à `AudioTrack` , où les arguments sont en grande partie des énumérations, les arguments équivalents dans `AudioRecord` sont des entiers. Il s’agit des tables suivantes :
...
5. Format audio &ndash; 8 bits ou 16 bits.

6. Taille de la mémoire tampon-en octets

Une fois le `AudioRecord` créé, sa méthode [StartRecording](xref:Android.Media.AudioRecord.StartRecording) est appelée. Il est maintenant prêt à commencer l’enregistrement. Le `AudioRecord` lit en continu la mémoire tampon audio pour l’entrée et écrit cette entrée dans un fichier audio.
