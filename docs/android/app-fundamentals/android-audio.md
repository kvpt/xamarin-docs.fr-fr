...
Les appareils mobiles modernes ont adopté des fonctionnalités qui, auparavant, auraient requis des éléments dédiés de l’équipement &ndash; les caméras, les lecteurs musicaux et les magnétoscopes. Pour cette raison, les frameworks multimédias sont devenus une fonctionnalité de première classe dans les API mobiles.
...
1. **Lecture audio avec MediaPlayer** &ndash; à l’aide de la classe intégrée `MediaPlayer` pour lire l’audio, y compris les fichiers audio locaux et les fichiers audio diffusés avec la classe `AudioTrack`.

2. **Enregistrement** des &ndash; audio à l’aide de la classe intégrée `MediaRecorder` pour enregistrer l’audio.

3. L' **utilisation de notifications audio** &ndash; l’utilisation de notifications audio pour créer des applications bien connues qui répondent correctement aux événements (par exemple, les appels téléphoniques entrants) en suspendant ou en annulant leurs sorties audio.

4. L' **utilisation de l’audio de bas niveau** &ndash; la lecture audio à l’aide de la classe `AudioTrack` en écrivant directement dans des mémoires tampons. Enregistrement audio à l’aide de la classe `AudioRecord` et lecture directe à partir de mémoires tampons.
...
Il est nécessaire de demander les autorisations de `RECORD_AUDIO` dans **fichier AndroidManifest. xml**:

![Section des autorisations requises du manifeste Android avec enregistrement\_AUDIO activé](android-audio-images/image01.png)
...
`MediaPlayer` pouvez lire des fichiers locaux ou distants en passant le chemin d’accès au fichier. Toutefois, `MediaPlayer` est très sensible à l’État et l’appel de l’une de ses méthodes dans un état incorrect entraîne la levée d’une exception. Il est important d’interagir avec `MediaPlayer` dans l’ordre indiqué ci-dessous pour éviter les erreurs.

### <a name="initializing-and-playing"></a>Initialisation et exécution

La lecture audio avec `MediaPlayer` nécessite la séquence suivante :
...
La classe [MediaRecorder](xref:Android.Media.MediaRecorder) pour `MediaPlayer` l’enregistrement audio dans Android est la classe. À l’instar de la `MediaPlayer`, elle est sensible à l’État et passe par plusieurs États pour atteindre le point où elle peut démarrer l’enregistrement. Pour pouvoir enregistrer l’audio, l’autorisation `RECORD_AUDIO` doit être définie. Pour obtenir des instructions sur la façon de définir des autorisations d’application, consultez [utilisation de fichier AndroidManifest. xml](~/android/platform/android-manifest.md).

### <a name="initializing-and-recording"></a>Initialisation et enregistrement

L’enregistrement audio avec le `MediaRecorder` nécessite les étapes suivantes :
...
Pour arrêter l’enregistrement, appelez la méthode `Stop` sur le `MediaRecorder`:
...
Lorsque l' `MediaRecorder` n’est plus nécessaire, ses ressources doivent être libérées en appelant la méthode [Release](xref:Android.Media.MediaRecorder.Release) :
...
Le focus audio est une idée similaire et empêche la lecture ou l’enregistrement audio de plusieurs applications en même temps. Il est plus compliqué que le focus clavier, car il est volontaire &ndash; l’application peut ignorer ce fait qu’il ne possède pas actuellement le focus audio et s’exécute indépendamment &ndash; et parce qu’il y a différents types de focus audio qui peuvent être demandés. Par exemple, si le demandeur est supposé uniquement lire de l’audio pendant un très bref laps de temps, il peut demander un focus temporaire.
...
L’inscription du rappel `FocusChangeListener` à partir du `IOnAudioChangeListener` est une partie importante de l’obtention et de la libération du focus audio. Cela est dû au fait que l’octroi du focus audio peut être différé jusqu’à une date ultérieure. Par exemple, une application peut demander à écouter de la musique pendant un appel téléphonique en cours. Le focus audio n’est pas accordé tant que l’appel téléphonique n’est pas terminé.

Pour cette raison, l’objet de rappel est passé en tant que paramètre à la méthode `GetAudioFocus` du `AudioManager`, et c’est cet appel qui inscrit le rappel. Si le focus audio est initialement refusé, mais accordé ultérieurement, l’application est informée en appelant `OnAudioFocusChange` sur le rappel. La même méthode est utilisée pour indiquer à l’application que le focus audio est retiré.

Lorsque l’application a terminé d’utiliser les ressources audio, elle appelle la méthode `AbandonFocus` de la `AudioManager`et passe à nouveau dans le rappel. Cela annule l’inscription du rappel et libère les ressources audio, afin que d’autres applications puissent obtenir le focus audio.
...
1. Obtenez un handle vers le service système `AudioManager`.

2. Créez une instance de la classe de rappel.

3. Demandez les ressources audio de l’appareil en appelant la méthode `RequestAudioFocus` sur le `AudioManager`. Les paramètres sont l’objet de rappel, le type de flux (musique, appel vocal, sonnerie, etc.) et le type de droit d’accès demandé (les ressources audio peuvent être demandées momentanément ou pendant une période indéterminée, par exemple).

4. Si la demande est accordée, la méthode `playMusic` est appelée immédiatement, et le contenu audio commence à être lu.
...
Lorsque la lecture de la piste est terminée, la méthode `AbandonFocus` sur `AudioManager` est appelée. Cela permet à une autre application d’obtenir les ressources audio de l’appareil. D’autres applications recevront une notification de ce changement de focus audio s’ils ont inscrit leurs propres écouteurs.
...
La classe [AudioTrack](xref:Android.Media.AudioTrack) utilise les API audio de bas niveau pour l’enregistrement et est l’équivalent de bas niveau de la classe `MediaPlayer`.

#### <a name="initializing-and-playing"></a>Initialisation et exécution

Pour lire de l’audio, une nouvelle instance de `AudioTrack` doit être instanciée. La liste d’arguments passée dans le [constructeur](xref:Android.Media.AudioTrack) spécifie comment lire l’exemple audio contenu dans la mémoire tampon. Les arguments sont les suivants :

1. Type de flux &ndash; voix, sonnerie, musique, système ou alarme.

2. Fréquence &ndash; le taux d’échantillonnage exprimé en Hz.

3. Configuration de canal &ndash; mono ou stéréo.

4. Format audio &ndash; encodage 8 bits ou 16 bits.

5. Taille de la mémoire tampon &ndash; en octets.

6. Mode mémoire tampon &ndash; la diffusion en continu ou statique.

Après la construction, la méthode [Play](xref:Android.Media.AudioTrack.Play) de `AudioTrack` est appelée afin de la configurer pour démarrer la lecture. L’écriture de la mémoire tampon audio sur le `AudioTrack` démarre la lecture :
...
Lorsque le `AudioTrack` n’est plus nécessaire, ses ressources doivent être libérées par l’appel de [Release](xref:Android.Media.AudioTrack.Release):
...
La classe [AudioRecord](xref:Android.Media.AudioRecord) est l’équivalent de `AudioTrack` côté enregistrement. Comme `AudioTrack`, il utilise directement des mémoires tampons, à la place des fichiers et des URI. Il requiert que l’autorisation `RECORD_AUDIO` soit définie dans le manifeste.
...
2. Type de flux &ndash; voix, sonnerie, musique, système ou alarme.

3. Fréquence &ndash; le taux d’échantillonnage exprimé en Hz.

4. Configuration de canal &ndash; mono ou stéréo.

5. Format audio &ndash; encodage 8 bits ou 16 bits.

6. Taille de la mémoire tampon-en octets

Une fois la `AudioRecord` construite, sa méthode [StartRecording](xref:Android.Media.AudioRecord.StartRecording) est appelée. Il est maintenant prêt à commencer l’enregistrement. Le `AudioRecord` lit en continu la mémoire tampon audio en entrée et écrit cette entrée dans un fichier audio.
...
Lorsque l’objet `AudioRecord` n’est plus nécessaire, l’appel de sa méthode [Release](xref:Android.Media.AudioRecord.Release) libère toutes les ressources qui lui sont associées :
