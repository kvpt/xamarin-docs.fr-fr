...
ms.locfileid: "91455129"
---
# <a name="touch-in-android"></a>Entrées tactiles dans Android

À l’instar d’iOS, Android crée un objet qui contient des données sur l’interaction physique de l’utilisateur avec l’écran &ndash; d’un `Android.View.MotionEvent` objet. Cet objet contient des données telles que l’action effectuée, l’endroit où la pression tactile a eu lieu, la pression appliquée, etc. Un `MotionEvent` objet décompose le déplacement vers jusqu’aux valeurs suivantes :
...
- *Assigner un gestionnaire `View.Touch` d’événements à* -La `Android.Views.View` classe a un `EventHandler<View.TouchEventArgs>` auquel les applications peuvent assigner un gestionnaire. Il s’agit d’un comportement .NET classique.

- *Implémentation `View.IOnTouchListener` * de -Les instances de cette interface peuvent être assignées à un objet de vue à l’aide de la vue. `SetOnListener` méthode. Cela équivaut de façon fonctionnelle à l’affectation d’un gestionnaire d’événements à l' `View.Touch` événement. S’il existe une logique commune ou partagée qui peut nécessiter de nombreuses vues différentes quand elles sont touchées, il est plus efficace de créer une classe et d’implémenter cette méthode que d’assigner à chaque vue son propre gestionnaire d’événements.

- *Remplacer `View.OnTouchEvent` * -Toutes les vues dans la sous-classe Android `Android.Views.View` . Quand une vue est touchée, Android appelle le `OnTouchEvent` et lui passe un `MotionEvent` objet en tant que paramètre.
...
Pour implémenter un détecteur de mouvements, une activité doit instancier une `GestureDetector` classe et fournir une instance de `IOnGestureListener` , comme l’illustre l’extrait de code suivant :
...
Lorsque cet événement est déclenché, l’activité demande `GestureLibrary` à un d’essayer et de faire correspondre le mouvement de l’utilisateur avec l’un des mouvements créés par l’outil geste. `GestureLibrary` retourne une liste d’objets de prédiction.

Chaque objet de prédiction contient un score et un nom de l’un des mouvements dans le `GestureLibrary` . Plus le score est élevé, plus le geste nommé dans la prédiction correspond au geste dessiné par l’utilisateur.
