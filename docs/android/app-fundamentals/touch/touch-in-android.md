...
ms.locfileid: "73024318"
---
# <a name="touch-in-android"></a>Entrées tactiles dans Android

À l’instar d’iOS, Android crée un objet qui contient des données sur l’interaction physique de l’utilisateur avec l’écran &ndash; un objet `Android.View.MotionEvent`. Cet objet contient des données telles que l’action effectuée, l’endroit où la pression tactile a eu lieu, la pression appliquée, etc. Un objet `MotionEvent` décompose le déplacement vers jusqu’aux valeurs suivantes :
...
- Ensemble de valeurs d’axe qui décrivent la position du `MotionEvent` et d’autres propriétés de mouvement, telles que l’endroit où le toucher a lieu, le moment où le toucher a eu lieu et la pression utilisée.
...
L’objet `MotionEvent` est passé à une méthode appropriée dans une application. Il existe trois façons pour une application Xamarin. Android de répondre à un événement tactile :

- *Assigner un gestionnaire d’événements à `View.Touch`* : la classe `Android.Views.View` a un `EventHandler<View.TouchEventArgs>` auquel les applications peuvent assigner un gestionnaire. Il s’agit d’un comportement .NET classique.

- L' *implémentation de `View.IOnTouchListener`* -instances de cette interface peut être assignée à un objet de vue à l’aide de la vue. méthode `SetOnListener`. Cela équivaut de façon fonctionnelle à l’assignation d’un gestionnaire d’événements à l’événement `View.Touch`. S’il existe une logique commune ou partagée qui peut nécessiter de nombreuses vues différentes quand elles sont touchées, il est plus efficace de créer une classe et d’implémenter cette méthode que d’assigner à chaque vue son propre gestionnaire d’événements.

- *Remplace* les vues `View.OnTouchEvent`-All dans la sous-classe Android `Android.Views.View`. Quand une vue est touchée, Android appelle le `OnTouchEvent` et lui transmet un objet `MotionEvent` en tant que paramètre.
...
Android fournit l’espace de noms `Android.Gestures` spécifiquement pour la gestion et la réponse aux gestes. Au cœur de tous les gestes se trouve une classe spéciale appelée `Android.Gestures.GestureDetector`. Comme son nom l’indique, cette classe écoute les mouvements et les événements en fonction des `MotionEvents` fournis par le système d’exploitation.

Pour implémenter un détecteur de mouvements, une activité doit instancier une classe `GestureDetector` et fournir une instance de `IOnGestureListener`, comme illustré dans l’extrait de code suivant :
...
Lorsqu’une instance de `GestureDetector` identifie un mouvement d’intérêt, elle avertit l’activité ou l’application en déclenchant un événement ou via un rappel fourni par `GestureDetector.IOnGestureListener`.
...
Le `GestureOverlayView` a plusieurs événements qui seront déclenchés pendant le processus de dessin d’un mouvement. L’événement le plus intéressant est `GesturePerformed`. Cet événement est déclenché lorsque l’utilisateur a terminé de dessiner son geste.

Lorsque cet événement est déclenché, l’activité demande à un `GestureLibrary` d’essayer et de faire correspondre le mouvement avec l’utilisateur avec l’un des mouvements créés par l’outil geste. `GestureLibrary` renvoie une liste d’objets de prédiction.

Chaque objet de prédiction contient un score et un nom de l’un des mouvements de l' `GestureLibrary`. Plus le score est élevé, plus le geste nommé dans la prédiction correspond au geste dessiné par l’utilisateur.
