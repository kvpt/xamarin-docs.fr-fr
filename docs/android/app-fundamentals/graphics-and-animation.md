...
- **Ressources** &ndash; dessinables Celles-ci sont utilisées pour créer des graphiques personnalisés par programmation ou (plus généralement) en incorporant des instructions de dessin dans des fichiers XML. Les ressources pouvant être dessinées sont généralement définies en tant que fichiers XML qui contiennent des instructions ou des actions pour Android pour le rendu d’un graphique 2D. 
...
- [ShapeDrawable](https://developer.android.com/guide/topics/resources/drawable-resource.html#Shape) &ndash; Il s’agit d’un objet pouvant être dessiné qui dessine une forme géométrique primitive et qui applique un ensemble limité d’effets graphiques sur cette forme. Ils sont très utiles pour des tâches telles que la personnalisation des boutons ou la définition de l’arrière-plan de TextViews. Nous verrons un exemple de la façon d’utiliser un `ShapeDrawable` plus loin dans cet article.
...
Il existe deux façons d’obtenir un `Canvas` objet. La première consiste à définir un objet [bitmap](xref:Android.Graphics.Bitmap) , puis à instancier un `Canvas` objet avec lui. Par exemple, l’extrait de code suivant crée un nouveau canevas avec une bitmap sous-jacente :
...
L’autre façon d’obtenir un `Canvas` objet est que la méthode de rappel [OnDraw](xref:Android.Views.View.OnDraw*) qui est fournie par la classe de base [View](xref:Android.Views.View) . Android appelle cette méthode lorsqu’il décide qu’une vue doit se dessiner elle-même et passe dans un `Canvas` objet pour que la vue fonctionne.
...
- [Canvas. DrawText](xref:Android.Graphics.Canvas.DrawText*) &ndash; dessine le texte sur le canevas avec la couleur spécifiée. Le texte est dessiné à l’emplacement `x,y` .
...
- **Animation** &ndash; de propriété Les animations de propriété ont été introduites dans Android 3,0. Elles permettent à une application d’animer presque n’importe quoi. Les animations de propriété peuvent être utilisées pour modifier toute propriété d’un objet, même si cet objet n’est pas visible à l’écran.

- **Animation** &ndash; dessinable Il s’agit d’une ressource dessinable spéciale qui est utilisée pour appliquer un effet d’animation très simple aux dispositions.
...
Par défaut, toutes les animations d’un fichier XML sont appliquées simultanément. Pour faire en sorte que les animations se produisent de façon séquentielle, définissez l' `android:startOffset` attribut sur l’un des éléments définis ci-dessus.
...
- `AccelerateInterpolator`/ `DecelerateInterpolator` &ndash; Ces interpolateurs augmentent ou diminuent le taux de modification dans une animation.
...
L’animation peut être appliquée par programmation à une vue en la gonflant, puis en l’appliquant à une vue. Android fournit la classe d’assistance `Android.Views.Animations.AnimationUtils` qui permet d’augmenter une ressource d’animation et de retourner une instance de `Android.Views.Animations.Animation` . Cet objet est appliqué à une vue en appelant `StartAnimation` et en passant l' `Animation` objet. L’extrait de code suivant en montre un exemple :
...
Si la propriété animée n’est ni une `float` `int` couleur, ni une couleur, les applications peuvent créer leur propre évaluateur en implémentant l' `ITypeEvaluator` interface. (L’implémentation d’évaluateurs personnalisés dépasse le cadre de cette rubrique.)
...
Une fois cette opération terminée, la durée de l' `ValueAnimator` instance doit être définie, puis elle peut être démarrée. L’exemple suivant montre comment animer une valeur de 0 à 1 sur l’étendue de 1000 millisecondes :
...
Maintenant que nous avons une compréhension de `ValueAnimator` , vous pouvez en savoir plus sur le `ObjectAnimator` .

#### <a name="using-the-objectanimator"></a>Utilisation de ObjectAnimator

[ObjectAnimator](xref:Android.Animation.ObjectAnimator) est une sous-classe de `ViewAnimator` qui associe le moteur de minutage et le calcul de valeur de `ValueAnimator` avec la logique requise pour connecter des gestionnaires d’événements. Requiert que les applications associent `ValueAnimator` explicitement un gestionnaire d’événements &ndash; `ObjectAnimator` s’occupent de cette étape pour nous.

L’API pour `ObjectAnimator` est très similaire à l’API pour `ViewAnimator` , mais vous devez fournir l’objet et le nom de la propriété à mettre à jour. L’exemple suivant montre un exemple d’utilisation de `ObjectAnimator` :
...
Les ressources pouvant être dessinées sont définies dans un fichier XML qui a un `<animation-list>` élément en tant qu’élément racine et une série d' `<item>` éléments qui définissent chaque frame dans l’animation. Ce fichier XML est stocké dans le `/Resource/drawable` dossier de l’application. Le code XML suivant est un exemple d’animation dessinable :
...
- [Démonstration d’animation (exemple)](/samples/xamarin/monodroid-samples/animationdemo)
...
- [Canevas](xref:Android.Graphics.Canvas)
