...
- ***Disposition*** &ndash; de base format de présentation compact et fixe.
...
- **Catégorie** &ndash; Informe le système Comment gérer la notification dans différentes circonstances, par exemple quand l’appareil est en mode *ne pas déranger* .
...
3. Appelez la méthode de [génération](xref:Android.App.Notification.Builder.Build) de l' `NotificationCompat.Builder` objet pour instancier un objet de notification.
...
Si vous souhaitez que votre notification émette également un son, vous pouvez appeler la méthode [SetDefaults](xref:Android.App.Notification.Builder.SetDefaults*) du générateur de notifications et transmettre l' `NotificationDefaults.Sound` indicateur :
...
Une fois que vous avez créé un objet de notification, il est possible de définir des propriétés de notification sur l’objet de notification (au lieu de les configurer à l’avance par le biais des `NotificationCompat.Builder` méthodes). Par exemple, au lieu d’appeler la `SetDefaults` méthode pour activer les vibrations sur une notification, vous pouvez modifier directement l’indicateur de bit de la propriété [par défaut](xref:Android.App.Notification.Defaults) de la notification :
...
Si vous souhaitez mettre à jour le contenu d’une notification après sa publication, vous pouvez réutiliser l' `NotificationCompat.Builder` objet existant pour créer un nouvel objet de notification et publier cette notification avec l’identificateur de la dernière notification. Par exemple :
...
Dans cet exemple, l' `NotificationCompat.Builder` objet existant est utilisé pour créer un nouvel objet de notification avec un titre et un message différents.
...
L’extrait de code suivant montre comment créer une notification avec un `PendingIntent` qui lance l’activité de l’application d’origine `MainActivity` :
...
Ce message extrait, « Greetings from MainActivity ! », s’affiche à l' `SecondActivity` écran, comme indiqué dans la capture d’écran ci-dessus. Quand l’utilisateur appuie sur le bouton **précédent** alors qu’il se trouve dans `SecondActivity` , la navigation en sort de l’application et revient à l’écran précédant le lancement de l’application.
...
Voici un exemple de notification Android 5,0 de base &ndash; . elle affiche uniquement la petite icône d’application :
...
Pour créer une notification de *texte volumineux* , vous devez instancier un `NotificationCompat.Builder` objet, comme avant, puis instancier et ajouter un objet [BigTextStyle](xref:Android.App.Notification.BigTextStyle) à l' `NotificationCompat.Builder` objet. Voici un exemple :
...
Le style d' *image* (également appelé style d' *image Big* ) est un format de notification développé que vous pouvez utiliser pour afficher une image dans le corps d’une notification. Par exemple, une application de capture d’écran ou une application photo peut utiliser le style de notification d' *image* pour fournir à l’utilisateur une notification de la dernière image capturée. Notez que la hauteur maximale de la notification d' *image* est de 256 DP &ndash; le redimensionnement de l’image pour s’ajuster à cette restriction de hauteur maximale, dans les limites de la mémoire disponible.
...
Notez que lorsque la notification est affichée au format compact, elle affiche le texte de notification (le texte qui est transmis à la méthode du générateur de notification `SetContentText` , comme indiqué plus haut). Toutefois, lorsque la notification est développée pour afficher l’image, elle affiche le texte de résumé au-dessus de l’image.
...
Pour créer une notification de *boîte de réception* , vous devez instancier un `NotificationCompat.Builder` objet, comme avant, et ajouter un objet [InboxStyle](xref:Android.App.Notification.InboxStyle) à `NotificationCompat.Builder` . Voici un exemple :
...
- `NotificationVisibility.Private`&ndash;Seules les informations essentielles sont affichées sur l’écran verrouillage sécurisé (par exemple, l’icône de notification et le nom de l’application qui l’a publié), mais le reste des détails de la notification est masqué. Toutes les notifications ont par défaut la valeur `NotificationVisibility.Private` .
...
Pour définir la visibilité d’une notification, les applications appellent la `SetVisibility` méthode de l' `NotificationCompat.Builder` objet, en passant le paramètre de visibilité. Par exemple, cet appel à `SetVisibility` rend la notification `Private` :
...
Dans cet exemple, le **Framework cible** de l’application est défini sur Android 5,0 et la **version Android minimale** est définie sur **Android 4,1 (niveau d’API 16)**. Étant donné que `SetCategory` est disponible au niveau de l’API 21 et versions ultérieures, cet exemple de code appellera `SetCategory` uniquement lorsqu’il est disponible, &ndash; il n’appelle pas `SetCategory` lorsque le niveau d’API est inférieur à 21.

### <a name="lock-screen-visibility"></a>Visibilité de l’écran de verrouillage

Android ne prenant pas en charge les notifications de verrouillage de l’écran avant Android 5,0 (niveau d’API 21), `NotificationCompat.Builder` ne prend pas en charge la `SetVisibility` méthode. Comme expliqué ci-dessus `SetCategory` , votre code peut vérifier le niveau d’API au moment de l’exécution et appeler `SetVisiblity` uniquement lorsqu’il est disponible :
