...
Ajoutez l' `using` instruction suivante à **MainActivity.cs** , &ndash; ce qui permet d’accéder facilement aux méthodes de la classe basée sur Java `Picasso` qui réside dans la bibliothèque de liaisons :
