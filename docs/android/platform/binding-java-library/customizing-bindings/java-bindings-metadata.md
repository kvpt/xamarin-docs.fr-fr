...
Lorsque le générateur de liaisons Xamarin. Android identifie `onXXX` une méthode setter pour un _type d’écouteur_, un `EventArgs` événement et une C# sous-classe sont générés pour prendre en charge une API d’arôme .net pour le modèle d’écouteur basé sur Java. À titre d’exemple, considérez la classe Java et la méthode suivantes :
