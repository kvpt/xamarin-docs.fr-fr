...
Les liaisons à la plateforme Android sont contenues dans l’assembly `Mono.Android.dll`. Cet assembly contient l’ensemble de la liaison pour la consommation des API Android et la communication avec la machine virtuelle du runtime Android.
...
- [java. util. Set\<E >](https://developer.android.com/reference/java/util/Set.html) est mappé au type de système [ICollection\<t >](xref:System.Collections.Generic.ICollection`1), classe d’assistance [Android. Runtime. JavaSet\<t >](xref:Android.Runtime.JavaSet`1).

- [java. util. List\<E >](https://developer.android.com/reference/java/util/List.html) correspond au type de système [IList\<t >](xref:System.Collections.Generic.IList`1), classe d’assistance [Android. Runtime. JavaList\<t >](xref:Android.Runtime.JavaList`1).
...
- [java. util. Collection\<E >](https://developer.android.com/reference/java/util/Collection.html) est mappé au type de système [ICollection\<t >](xref:System.Collections.Generic.ICollection`1), classe d’assistance [Android. Runtime. JavaCollection\<t >](xref:Android.Runtime.JavaCollection`1).
...
Vous pouvez passer toute implémentation d’interface à une méthode Android acceptant cette interface, par exemple passer un `List<int>` au constructeur [ArrayAdapter&lt;int&gt;(Context, int, IList&lt;int&gt;)](xref:Android.Widget.ArrayAdapter`1) . *Toutefois*, pour toutes les implémentations *à l’exception* des implémentations Android. Runtime, vous devez *copier* la liste de la machine virtuelle mono dans la machine virtuelle du runtime Android. Si la liste est modifiée par la suite dans le runtime Android (par exemple, en appelant le [ArrayAdapter&lt;t&gt;. Méthode Add (T)](xref:Android.Widget.ArrayAdapter`1.Add*) ), ces modifications ne seront *pas* visibles dans le code managé. Si un `JavaList<int>` été utilisé, ces modifications sont visibles.
...
- La paire de méthodes Java `T getFoo()` et `void setFoo(T)` sont transformées en la propriété `Foo`. Exemple : [Activity. astent](xref:Android.App.Activity.Intent).
...
1. A un préfixe `set`, par exemple, [ *Set*OnClickListener](xref:Android.Views.View.SetOnClickListener*).

1. A un type de retour `void`.
...
En outre, si la méthode d’interface d’écouteur a un type de retour **booléen** au lieu de **void**, la sous-classe *EventArgs* générée contient une propriété *gérée* . La valeur de la propriété *gérée* est utilisée comme valeur de retour pour la méthode d' *écouteur* , et la valeur par défaut est `true`.

Par exemple, la méthode Android [View. setOnKeyListener ()](xref:Android.Views.View.SetOnKeyListener*) accepte l’interface [View. OnKeyListener](xref:Android.Views.View.IOnKeyListener) , tandis que la méthode [View. OnKeyListener. onKey (View, int, KeyEvent)](xref:Android.Views.View.IOnKeyListener.OnKey*) a un type de retour booléen. Xamarin. Android génère un événement [View. KeyPress](xref:Android.Views.View.KeyPress) correspondant, qui est une [vue EventHandler&lt;View. KeyEventArgs&gt;](xref:Android.Views.View.KeyEventArgs).
...
Toutes les interfaces d’écouteurs implémentent le [`Android.Runtime.IJavaObject`](xref:Android.Runtime.IJavaObject)
...
L’interface `Runnable` contient une seule méthode void, [Run ()](xref:Java.Lang.Runnable.Run). Il se prête donc à la liaison en C# tant que délégué [System. action](xref:System.Action) . Nous avons fourni des surcharges dans la liaison qui acceptent un paramètre `Action` pour tous les membres de l’API qui consomment un `Runnable` dans l’API native, par exemple [Activity. RunOnUiThread ()](xref:Android.App.Activity.RunOnUiThread*) et [View.post ()](xref:Android.Views.View.Post*).
...
Notez la manière dont `CubeWallpaper.CubeEngine` est imbriqué dans `CubeWallpaper`, `CubeWallpaper` hérite de la classe conteneur de `WallpaperService.Engine`, et `CubeWallpaper.CubeEngine` a un constructeur qui prend le type déclarant--`CubeWallpaper` dans ce cas, le tout comme indiqué ci-dessus.
...
Par exemple, un exemple d’application Android contenant une disposition d’interface utilisateur (`main.axml`), une chaîne de table d’internationalisation (`strings.xml`) et certaines icônes (`drawable-*/icon.png`) conservent ses ressources dans le répertoire « Resources » de l’application :
...
Vous devez ensuite utiliser `Resource.Drawable.icon` pour référencer le fichier `drawable/icon.png` ou `Resource.Layout.main` pour référencer le fichier `layout/main.xml`, ou `Resource.String.first_string` pour référencer la première chaîne dans le fichier de dictionnaire `values/strings.xml`.
