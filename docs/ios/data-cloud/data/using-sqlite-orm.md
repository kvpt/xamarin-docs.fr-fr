...
1. **Ajoutez une instruction using** : ajoutez l’instruction suivante aux fichiers C# où l’accès aux données est requis :
...
**iOS**
 ...
 [![exemple SQLite.NET iOS](using-sqlite-orm-images/image2-sml.png)](using-sqlite-orm-images/image2-sml.png#lightbox)
...
Pour cela, vous devez avoir ajouté SQLite à votre projet, comme [nous](#Usage)l’avons mis en surbrillance ici. Notez que la table de base de données SQLite est définie en ajoutant des attributs à une classe (la `Stock` classe) au lieu d’une commande CREATE TABLE.
...
- **[Colonne (nom)]** &ndash; Le `name` paramètre définit le nom de la colonne de base de données sous-jacente.
...
- **En \<T> savoir** plus : Tente de récupérer un objet à l’aide de la clé primaire.
- **Table \<T> ** : Retourne tous les objets de la table.
- **Delete** : supprime un objet à l’aide de sa clé primaire.
- **Requête \<T> ** -Exécuter une requête SQL qui retourne un nombre de lignes (sous la forme d’objets).
- **Execute** : utilisez cette méthode (et non pas `Query` ) lorsque vous ne vous attendez pas à revenir sur les lignes de SQL (comme les instructions INSERT, Update et Delete).
...
Les méthodes qui retournent des collections prennent en charge IEnumerable \<T> , ce qui vous permet d’utiliser LINQ pour interroger ou trier le contenu d’une table. Le code suivant montre un exemple utilisant LINQ pour filtrer toutes les entrées qui commencent par la lettre « A » :
...
Pour modifier le mode de thread, appelez `SqliteConnection.SetConfig` qui se trouve dans l' `Mono.Data.Sqlite` espace de noms. Par exemple, cette ligne de code configure SQLite pour le mode **sérialisé** :
