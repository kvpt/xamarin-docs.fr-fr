...
ms.locfileid: "73021902"
---
# <a name="editing-tables-with-xamarinios"></a>Modification des tables avec Xamarin. iOS

Les fonctionnalités d’édition de table sont activées en substituant des méthodes dans une sous-classe `UITableViewSource`. Le comportement d’édition le plus simple est le mouvement de balayage à supprimer qui peut être implémenté avec une seule substitution de méthode.
...
Ces méthodes sont implémentées dans la classe `TableSource` suit :
...
Pour cet exemple, la `UITableViewSource` a été mise à jour pour utiliser une `List<TableItem>` (au lieu d’un tableau de chaînes) comme source de données, car elle prend en charge l’ajout et la suppression d’éléments de la collection.
...
L’implémentation des trois premières méthodes est relativement simple, à moins que vous souhaitiez utiliser la `indexPath` pour modifier le comportement de lignes spécifiques, il suffit de coder en dur les valeurs de retour pour la table entière.
...
L’implémentation de `MoveRow` est un peu plus compliquée, car elle doit modifier la structure de données sous-jacente pour qu’elle corresponde à la nouvelle commande. Étant donné que les données sont implémentées en tant que `List` le code ci-dessous supprime l’élément de données à son ancien emplacement et l’insère au nouvel emplacement. Si les données ont été stockées dans une table de base de données SQLite avec une colonne « Order » (par exemple), cette méthode doit plutôt effectuer des opérations SQL pour réorganiser les nombres de cette colonne.
...
- **WillBeginTableEditing** : lorsque le bouton **modifier** est touché, il appelle `SetEditing` pour mettre la table en mode édition. Cela déclenche la méthode WillBeginTableEditing où nous affichons la ligne **(Ajouter nouveau)** à la fin de la table pour agir en tant que « bouton d’insertion ». 
- **DidFinishTableEditing** : lorsque le bouton terminé est touché `SetEditing` est rappelé pour désactiver le mode édition. L’exemple de code supprime la ligne **(Ajouter nouveau)** de la table lorsque la modification n’est plus nécessaire. 
...
Ce modèle d’interface utilisateur d’insertion de ligne n’est pas utilisé très souvent. Toutefois, vous pouvez également utiliser les méthodes `UITableView.BeginUpdates` et `EndUpdates` pour animer l’insertion ou la suppression de cellules dans une table. La règle d’utilisation de ces méthodes est que la différence de valeur retournée par `RowsInSection` entre les appels `BeginUpdates` et `EndUpdates` doit correspondre au nombre net de cellules ajoutées/supprimées avec les méthodes `InsertRows` et `DeleteRows`. Si la source de source sous-jacente n’est pas modifiée pour correspondre aux insertions/suppressions sur la table, une erreur se produit.
