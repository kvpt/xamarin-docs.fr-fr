...
L’article explique comment implémenter et utiliser les `UIPickerView` `UIDatePicker` contrôles et.
...
classe ou substituez les [`GetView`](xref:UIKit.UIPickerViewModel.GetView(UIKit.UIPickerView,System.nint,System.nint,UIKit.UIView)) [`GetRowHeight`](xref:UIKit.UIPickerViewModel.GetRowHeight(UIKit.UIPickerView,System.nint)) méthodes et dans le `UIPickerViewModel` .
...
> [!TIP]
> Il est possible d’effectuer un cast explicite `DateTime` d’un vers un `NSDate` :
>
> ```csharp
...
##### <a name="uidatepickermodetime"></a>UIDatePickerMode. heure
...
![UIDatePickerMode. heure](picker-images/image8.png "UIDatePickerMode. heure")
...
L’ordre des sélecteurs dépend des paramètres régionaux du sélecteur de dates, qui utilise par défaut les paramètres régionaux du système. L’image ci-dessus montre la disposition des sélecteurs dans les `en_US` paramètres régionaux, mais la commande suivante change la valeur de l’ordre en jour | Mois | Year
...
> [!TIP]
> Utilisez la `Date` propriété pour capturer la valeur d’un sélecteur de dates en mode `UIDatePickerMode.Time` , `UIDatePickerMode.Date` ou `UIDatePickerMode.DateAndTime` . Cette valeur est stockée sous la forme d’un `NSDate` .
...
> [!NOTE]
> `DateFormat`et `DateStyle` / `TimeStyle` fournissent différentes méthodes de spécification de la mise en forme de la date et de l’heure. Les propriétés les plus récentes déterminent la sortie du formateur de date.

## <a name="related-links"></a>Liens connexes

- [PickerControl (exemple)](/samples/xamarin/ios-samples/pickercontrol)