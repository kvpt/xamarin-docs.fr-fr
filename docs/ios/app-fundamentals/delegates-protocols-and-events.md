...
[ ![ Exemple d’application Map simple qui ajoute une annotation à un mappage](delegates-protocols-and-events-images/01-map-sml.png)](delegates-protocols-and-events-images/01-map.png#lightbox) 
 [ ![ un exemple d’annotation ajouté à une carte](delegates-protocols-and-events-images/04-annotation-with-callout-sml.png)](delegates-protocols-and-events-images/04-annotation-with-callout.png#lightbox)
...
Vous pouvez également l’implémenter avec une méthode anonyme de style C# 2,0 comme celle-ci :
...
[![Un bouton ajouté dans le concepteur iOS](delegates-protocols-and-events-images/02-interface-builder-outlet-sml.png)](delegates-protocols-and-events-images/02-interface-builder-outlet.png#lightbox)
...
[![Fichier code-behind UIViewControllers](delegates-protocols-and-events-images/03-interface-builder-action-sml.png)](delegates-protocols-and-events-images/03-interface-builder-action.png#lightbox)
...
[![Un message est écrit sur la console lorsque l’utilisateur clique sur le bouton.](delegates-protocols-and-events-images/05-interface-builder-action-sml.png)](delegates-protocols-and-events-images/05-interface-builder-action.png#lightbox)
...
Si vous souhaitez intercepter des événements à partir de Uicontrolextensions, vous disposez d’une gamme d’options : de l’utilisation des fonctions lambda et des fonctions déléguées C# à l’utilisation des API objective-C de bas niveau.
...
Les événements C# pour les indicateurs UIControlEvent ont un mappage un-à-un avec des indicateurs individuels. Si vous souhaitez que la même partie du code gère deux événements ou plus, utilisez la `UIControl.AddTarget` méthode :
...
Un protocole est une fonctionnalité du langage Objective-C qui fournit une liste de déclarations de méthode. Il a un objectif similaire à une interface en C#, mais la principale différence réside dans le fait qu’un protocole peut avoir des méthodes facultatives. Les méthodes facultatives ne sont pas appelées si la classe qui adopte un protocole ne les implémente pas. En outre, une classe unique en Objective-C peut implémenter plusieurs protocoles, tout comme une classe C# peut implémenter plusieurs interfaces.

Apple utilise des protocoles dans iOS pour définir les contrats des classes à adopter, tout en faisant abstraction de la classe d’implémentation de l’appelant, qui fonctionne comme une interface C#. Les protocoles sont utilisés à la fois dans les scénarios non délégués (comme avec l' `MKAnnotation` exemple suivant) et avec les délégués (comme indiqué plus loin dans ce document, dans la section délégués).
...
Toute classe peut fournir des données d’annotation en dérivant simplement de `MKAnnotation` , tant que la `Coordinate` propriété est implémentée au moins. Par exemple, voici un exemple de classe qui prend la coordonnée dans le constructeur et retourne une chaîne pour le titre :
...
Ici, la variable de mappage est une instance d’un `MKMapView` , qui est la classe qui représente le mappage lui-même. Le `MKMapView` utilise les `Coordinate` données dérivées de l' `SampleMapAnnotation` instance pour positionner la vue d’annotation sur la carte.
...
Dans la mesure où les interfaces C# ne prennent pas en charge les méthodes facultatives, Xamarin. iOS mappe les protocoles aux classes abstraites. Par conséquent, l’adoption d’un protocole en Objective-C s’effectue dans Xamarin. iOS en dérivant de la classe abstraite liée au protocole et en implémentant les méthodes requises. Ces méthodes seront exposées en tant que méthodes abstraites dans la classe. Les méthodes facultatives du protocole sont liées aux méthodes virtuelles de la classe C#.
...
Pour exécuter du code en réponse à l’utilisateur en appuyant sur l’annotation dans notre exemple, nous pouvons la sous-classe `MKMapViewDelegate` et assigner une instance à la `MKMapView` `Delegate` propriété de. Le `MKMapViewDelegate` protocole contient uniquement des méthodes facultatives.
Par conséquent, toutes les méthodes sont virtuelles qui sont liées à ce protocole dans la classe Xamarin. iOS `MKMapViewDelegate` . Lorsque l’utilisateur sélectionne une annotation, l' `MKMapView` instance envoie le `mapView:didSelectAnnotationView:` message à son délégué. Pour gérer cela dans Xamarin. iOS, nous devons remplacer la `DidSelectAnnotationView (MKMapView mapView, MKAnnotationView annotationView)` méthode dans la sous-classe MKMapViewDelegate comme suit :
...
La classe SampleMapDelegate présentée ci-dessus est implémentée en tant que classe imbriquée dans le contrôleur qui contient l' `MKMapView` instance. En Objective-C, vous verrez souvent que le contrôleur adopte plusieurs protocoles directement dans la classe. Toutefois, étant donné que les protocoles sont liés à des classes dans Xamarin. iOS, les classes qui implémentent des délégués fortement typés sont généralement incluses comme des classes imbriquées.

Avec l’implémentation de la classe déléguée en place, il vous suffit d’instancier une instance du délégué dans le contrôleur et de l’assigner à la `MKMapView` propriété de, `Delegate` comme illustré ici :
...
Pour utiliser un délégué faible pour accomplir la même chose, vous devez lier la méthode vous-même dans toute classe qui dérive de `NSObject` et l’assigner à la `WeakDelegate` propriété de `MKMapView` . Étant donné que la `UIViewController` classe dérive finalement de `NSObject` (comme chaque classe objective-C dans cocoatouch), nous pouvons tout simplement implémenter une méthode liée `mapView:didSelectAnnotationView:` directement dans le contrôleur et assigner le contrôleur à `MKMapView` , en `WeakDelegate` évitant la nécessité de la classe imbriquée supplémentaire. Le code ci-dessous illustre cette approche :
...
Par exemple, l’implémentation antérieure dans laquelle le a `MKMapViewDelegate` répondu à une annotation sélectionnée peut également être implémentée dans Xamarin. IOS à l’aide d’un événement .net. Dans ce cas, l’événement est défini dans `MKMapView` et appelé `DidSelectAnnotationView` . Elle aurait une `EventArgs` sous-classe de type `MKMapViewAnnotationEventsArgs` . La `View` propriété de vous `MKMapViewAnnotationEventsArgs` donne une référence à la vue d’annotation, à partir de laquelle vous pouvez effectuer la même implémentation que vous aviez précédemment, comme illustré ici :
...
Nous avons ensuite appris les protocoles objective-C, notamment la façon dont ils sont différents des interfaces C# et comment Xamarin. iOS les utilise. Enfin, nous avons examiné les délégués objective-C à partir d’une perspective Xamarin. iOS. Nous avons vu comment Xamarin. iOS prend en charge les délégués fortement et faiblement typés, et comment lier des événements .NET pour déléguer des méthodes.
