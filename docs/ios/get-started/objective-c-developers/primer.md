...
_Xamarin. iOS permet de partager le code indépendant de la plateforme écrit en C# entre les plateformes. Toutefois, les applications iOS existantes peuvent souhaiter tirer parti du code Objective-C qui a déjà été créé. Cet article sert de brève introduction pour les développeurs objective-C cherchant à passer à Xamarin et au langage C#._

les applications iOS et macOS développées en Objective-C peuvent tirer parti de Xamarin en tirant parti de C# dans des endroits où le code spécifique à la plateforme n’est pas obligatoire, ce qui permet d’utiliser un tel code sur des appareils non-Apple. Les éléments tels que les services web, l’analyse JSON et XML, ainsi que les algorithmes personnalisés, peuvent ensuite être utilisés à travers les différentes plateformes.
...
Ainsi, quand une instance d’un argument `UITextView` sera créée dans le code, la méthode sera disponible dans la liste de saisie semi-automatique, comme indiqué ci-dessous :
 ...
 ![Méthode disponible dans la saisie semi-automatique](primer-images/01-extensionmethodintellisense.png)
