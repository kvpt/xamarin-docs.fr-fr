...
[![Exemple d’exécution d’une opération de remise](handoff-images/handoff02.png)](handoff-images/handoff02.png#lightbox)
...
Pour qu’une activité soit passée à un autre des appareils de l’utilisateur, elle doit être encapsulée dans une instance `NSUserActivity` , marquée en tant qu' _activité actuelle_, avoir sa charge utile (les données utilisées pour effectuer la continuation) et l’activité doit ensuite être transmise à cet appareil.
...
[![Vue d’ensemble des activités continues de l’utilisateur](handoff-images/handoffinteractions.png)](handoff-images/handoffinteractions.png#lightbox)

Seules les applications qui partagent le même ID d’équipe de développeur et répondent à un _type d’activité_ donné sont éligibles pour la continuation. Une application définit les types d’activités qu’elle prend en charge sous la `NSUserActivityTypes` clé de son fichier **info. plist** . À partir de cela, un appareil continu choisit l’application pour effectuer la continuation en fonction de l’ID d’équipe, du type d’activité et éventuellement du titre de l' _activité_.
...
Une instance de `NSUserActivity` est identifiée par ses `ActivityType` `Title` Propriétés et. La `UserInfo` propriété Dictionary est utilisée pour transporter des informations sur l’état de l’activité. Affectez `NeedsSave` à la propriété la valeur `true` si vous souhaitez charger en différé les informations d’état via le `NSUserActivity` délégué de. Utilisez la `AddUserInfoEntries` méthode pour fusionner les nouvelles données à partir d’autres clients dans le `UserInfo` dictionnaire en fonction des besoins afin de conserver l’état de l’activité.

### <a name="the-nsuseractivitydelegate-class"></a>La classe NSUserActivityDelegate

`NSUserActivityDelegate`Est utilisé pour tenir à jour les informations contenues dans le dictionnaire d’un `NSUserActivity` `UserInfo` et synchroniser l’état actuel de l’activité. Quand le système a besoin que les informations de l’activité soient mises à jour (par exemple avant la continuation sur un autre périphérique), il appelle la `UserActivityWillSave` méthode du délégué.

Vous devrez implémenter la `UserActivityWillSave` méthode et apporter des modifications au `NSUserActivity` (par exemple `UserInfo` , `Title` , etc.) pour vous assurer qu’elle reflète toujours l’état de l’activité en cours. Lorsque le système appelle la `UserActivityWillSave` méthode, l' `NeedsSave` indicateur est effacé. Si vous modifiez l’une des propriétés de données de l’activité, vous devez réaffecter `NeedsSave` à la valeur `true` .

Au lieu d’utiliser la `UserActivityWillSave` méthode présentée ci-dessus, vous pouvez éventuellement avoir `UIKit` ou `AppKit` gérer l’activité de l’utilisateur automatiquement. Pour ce faire, définissez la propriété de l’objet du répondeur `UserActivity` et implémentez la `UpdateUserActivityState` méthode. Pour plus d’informations, consultez la section [prise en charge de la remise dans les répondeurs](#supporting-handoff-in-responders) ci-dessous.

### <a name="app-framework-support"></a>Prise en charge de l’infrastructure d’application

Les deux `UIKit` (IOS) et `AppKit` (OS X) offrent une prise en charge intégrée de la remise dans les `NSDocument` classes, répondeur ( `UIResponder` / `NSResponder` ) et `AppDelegate` . Bien que chaque système d’exploitation implémente un transfert légèrement différent, le mécanisme de base et les API sont les mêmes.
...
Les applications iOS et OS X basées sur des documents disposent automatiquement de la prise en charge de la remise intégrée. Pour activer cette prise en charge, vous devez ajouter une `NSUbiquitousDocumentUserActivityType` clé et une valeur pour chaque `CFBundleDocumentTypes` entrée dans le fichier **info. plist** de l’application.

Si cette clé est présente, `NSDocument` et `UIDocument` crée automatiquement des `NSUserActivity` instances pour les documents basés sur icloud du type spécifié. Vous devrez fournir un type d’activité pour chaque type de document pris en charge par l’application et plusieurs types de documents peuvent utiliser le même type d’activité. `NSDocument`Et `UIDocument` remplissent automatiquement la `UserInfo` propriété de `NSUserActivity` avec la `FileURL` valeur de leur propriété.

Sur OS X, le `NSUserActivity` géré par `AppKit` et associé aux répondeurs deviennent automatiquement l’activité en cours lorsque la fenêtre du document devient la fenêtre principale. Sur iOS, pour les `NSUserActivity` objets gérés par `UIKit` , vous devez appeler `BecomeCurrent` explicitement la méthode ou faire en sorte que la propriété du document soit `UserActivity` définie sur un `UIViewController` lorsque l’application passe au premier plan.

`AppKit` restaure automatiquement toutes les `UserActivity` Propriétés créées de cette façon sur OS X. Cela se produit si la `ContinueUserActivity` méthode retourne `false` ou si elle n’est pas implémentée. Dans ce cas, le document est ouvert avec la `OpenDocument` méthode de `NSDocumentController` et il reçoit un `RestoreUserActivityState` appel de méthode.
...
`UIKit`Et `AppKit` peuvent gérer automatiquement une activité utilisateur si vous la définissez en tant que propriété d’un objet répondeur `UserActivity` . Si l’État a été modifié, vous devez définir la `NeedsSave` propriété du répondeur `UserActivity` sur `true` . Le système enregistre automatiquement le quand cela est `UserActivity` nécessaire, après avoir donné à l’heure du répondeur la mise à jour de l’État en appelant sa `UpdateUserActivityState` méthode.

Si plusieurs répondeurs partagent une `NSUserActivity` instance unique, ils reçoivent un `UpdateUserActivityState` rappel lorsque le système met à jour l’objet d’activité utilisateur. Le répondeur doit appeler la `AddUserInfoEntries` méthode pour mettre à jour `NSUserActivity` le `UserInfo` dictionnaire de pour refléter l’état actuel de l’activité à ce stade. Le `UserInfo` dictionnaire est effacé avant chaque `UpdateUserActivityState` appel.

Pour se dissocier d’une activité, un répondeur peut affecter à sa propriété la valeur `UserActivity` `null` . Quand une instance gérée de l’infrastructure d’application n' `NSUserActivity` a plus de répondeurs ou de documents associés, elle est automatiquement invalidée.
...
La valeur de votre application `AppDelegate` est son point d’entrée principal lors du traitement d’une continuation de transfert. Lorsque l’utilisateur répond à une notification de remise, l’application appropriée est lancée (si elle n’est pas déjà en cours d’exécution) et la `WillContinueUserActivityWithType` méthode du `AppDelegate` sera appelée. À ce stade, l’application doit informer l’utilisateur que la continuation est en cours de démarrage.

L' `NSUserActivity` instance est remise lorsque la `AppDelegate` `ContinueUserActivity` méthode de est appelée. À ce stade, vous devez configurer l’interface utilisateur de l’application et continuer l’activité donnée.
...
4. Assurez-vous que le service **icloud** a été vérifié pour l’ID donné :
    ...
    [![Activer le service iCloud pour l’ID donné](handoff-images/provision01.png)](handoff-images/provision01.png#lightbox)
...
6. Cliquez sur **Configuration des profils de provisionnement**  >  **Development** et créez un nouveau profil de provisionnement de développement pour votre application :
    ...
    [![Créer un nouveau profil de provisionnement de développement pour l’application](handoff-images/provision02.png)](handoff-images/provision02.png#lightbox)
...
8. Modifiez les options de votre projet Xamarin. iOS et assurez-vous d’utiliser le profil de provisionnement que vous venez de créer :
    ...
    [![Sélectionner le profil de provisionnement que vous venez de créer](handoff-images/provision03.png)](handoff-images/provision03.png#lightbox)
9. Modifiez ensuite votre fichier **info. plist** et assurez-vous que vous utilisez l’ID d’application qui a été utilisé pour créer le profil de provisionnement :
    ...
    [![Définir l’ID de l’application](handoff-images/provision04.png)](handoff-images/provision04.png#lightbox)
10. Faites défiler jusqu’à la section **modes d’arrière-plan** et vérifiez les éléments suivants :

    [![Activer les modes d’arrière-plan requis](handoff-images/provision05.png)](handoff-images/provision05.png#lightbox)
...
Par exemple, nous allons créer un exemple d’application appelé **MonkeyBrowser** ([Téléchargez ici](/samples/xamarin/ios-samples/ios8-monkeybrowser)). Cette application présente quatre onglets, chacun avec une URL différente ouverte dans une vue de navigateur Web. L’utilisateur pourra continuer à accéder à tous les onglets d’un autre appareil iOS exécutant l’application.
...
[![Clé NSUserActivityTypes et identificateurs requis dans l’éditeur plist](handoff-images/type01.png)](handoff-images/type01.png#lightbox)
...
Pour notre exemple d’application, un nouveau `NSUserActivity` est créé chaque fois que l’utilisateur entre une nouvelle URL dans l’un des affichages de navigateurs Web avec onglets. Le code suivant stocke l’état d’un onglet donné :
...
Comme nous l’avons vu ci-dessus, la `UserInfo` propriété de la `NSUserActivity` classe est une `NSDictionary` paire clé-valeur utilisée pour définir l’état d’une activité donnée. Les valeurs stockées dans `UserInfo` doivent être de l’un des types suivants : `NSArray` , `NSData` , `NSDate` , `NSDictionary` , `NSNull` , `NSNumber` , `NSSet` , `NSString` ou `NSURL` . `NSURL` les valeurs de données qui pointent vers des documents iCloud sont automatiquement ajustées afin qu’elles pointent vers les mêmes documents sur un appareil de réception.

Dans l’exemple ci-dessus, nous avons créé un `NSMutableDictionary`  objet et l’avons renseigné avec une clé unique fournissant l’URL actuellement affichée par l’utilisateur sur l’onglet donné. La `AddUserInfoEntries` méthode de l’activité utilisateur a été utilisée pour mettre à jour l’activité avec les données qui seront utilisées pour restaurer l’activité sur l’appareil de réception :
...
Le transfert informe automatiquement les appareils iOS et OS X locaux qui se trouvent à proximité de l’appareil d’origine et qui sont connectés au même compte iCloud, de la disponibilité des activités utilisateur continues. Si l’utilisateur choisit de poursuivre une activité sur un nouvel appareil, le système lance l’application appropriée (selon l’ID d’équipe et le type d’activité) et les informations `AppDelegate` dont elle a besoin pour se produire.
...
Dans l’exemple ci-dessus, chaque contrôleur d’affichage s’inscrit auprès du `AppDelegate` et possède une `PreparingToHandoff` méthode publique qui affiche un indicateur d’activité et un message indiquant à l’utilisateur que l’activité va être transposée à l’appareil actuel. Exemple :
...
Le `ContinueUserActivity` du `AppDelegate` sera appelé pour continuer l’activité donnée. Là encore, à partir de notre exemple d’application :
...
La `PerformHandoff` méthode publique de chaque contrôleur d’affichage préforme réellement le transfert et restaure l’activité sur l’appareil actuel. Dans le cas de l’exemple, il affiche la même URL dans un onglet donné que l’utilisateur parcourt sur un autre appareil. Exemple :
...
Pour les applications basées sur un document, si vous n’implémentez pas la `ContinueUserActivity` méthode ou si elle retourne `false` , `UIKit` ou `AppKit` peut reprendre automatiquement l’activité. Pour plus d’informations, consultez la section [prise en charge du transfert dans les applications basées sur des documents](#supporting-handoff-in-document-based-apps) ci-dessous.
...
En cas d’échec, la `DidFailToContinueUserActivitiy` méthode du `AppDelegate` sera appelée. Par exemple :
...
Si le domaine spécifié correspond à la `WebpageURL` valeur d’une propriété, le transfert télécharge une liste des ID d’application approuvés à partir du site Web de ce domaine. Le site Web doit fournir une liste d’ID approuvés dans un fichier JSON signé nommé **Apple-App-site-Association** (par exemple, `https://company.com/apple-app-site-association` ).
...
Pour signer le fichier JSON (afin qu’il ait la bonne `Content-Type` valeur `application/pkcs7-mime` ), utilisez l’application **Terminal** et une `openssl` commande avec un certificat et une clé émis par une autorité de certification approuvée par iOS (voir [https://support.apple.com/kb/ht5012](https://support.apple.com/kb/ht5012) pour obtenir une liste). Par exemple :
...
L’application recevra toute activité dont le `WebpageURL` domaine est dans son `com.apple.developer.associated-domains` droit. Seuls les `http` `https` protocoles et prennent en charge, tout autre protocole lève une exception.
...
Comme indiqué ci-dessus, sur iOS et OS X, les applications basées sur les documents prendront en charge automatiquement le transfert de documents basés sur iCloud si le fichier **info. plist** de l’application contient une `CFBundleDocumentTypes` clé de `NSUbiquitousDocumentUserActivityType` . Par exemple :
...
L’objet d’activité utilisateur créé automatiquement (disponible par le biais de la propriété du document `UserActivity` ) peut être référencé par d’autres objets dans l’application et utilisé pour restaurer l’état lors de la continuation. Par exemple, pour suivre la sélection des éléments et la position du document. Vous devez définir cette `NeedsSave` propriété d’activités à `true` chaque fois que l’état change et mettre à jour le `UserInfo` dictionnaire dans la `UpdateUserActivityState` méthode.
...
Vous pouvez associer des répondeurs (hérités d' `UIResponder` iOS ou `NSResponder` de OS X) à des activités en définissant leurs `UserActivity` Propriétés. Le système enregistre automatiquement la `UserActivity` propriété aux moments appropriés, en appelant la méthode du répondeur `UpdateUserActivityState` pour ajouter les données actuelles à l’objet d’activité utilisateur à l’aide de la  `AddUserInfoEntriesFromDictionary` méthode.
...
L’application d’origine affecte la valeur `SupportsContinuationStreams` à la propriété de l' `NSUserActivity` instance `true` . Par exemple :
...
L’application réceptrice peut ensuite appeler la `GetContinuationStreams` méthode de `NSUserActivity` dans `AppDelegate` pour établir le flux. Par exemple :
...
Sur l’appareil d’origine, le délégué d’activité de l’utilisateur reçoit les flux en appelant sa `DidReceiveInputStream` méthode pour fournir les données requises pour continuer l’activité de l’utilisateur sur l’appareil de reprise.

Vous allez utiliser un `NSInputStream` pour fournir un accès en lecture seule aux données de flux, et un pour un `NSOutputStream` accès en écriture seule. Les flux doivent être utilisés dans un mode de requête et de réponse, où l’application réceptrice demande plus de données et l’application d’origine la fournit. Ainsi, les données écrites dans le flux de sortie sur l’appareil d’origine sont lues à partir du flux d’entrée sur le périphérique continu, et vice versa.
...
- Si votre application doit mettre à jour l’état d’une activité utilisateur ( `NSUserActivity` ), affectez la valeur `NeedsSave` à la propriété `true` . À des moments opportuns, le transfert appellera la `UserActivityWillSave` méthode du délégué afin que vous puissiez mettre à jour le `UserInfo` dictionnaire en fonction des besoins.
- Étant donné que le processus de remise peut ne pas s’initialiser instantanément sur l’appareil de réception, vous devez implémenter le `AppDelegate` `WillContinueUserActivity` et informer l’utilisateur qu’une continuation est sur le le début.

## <a name="example-handoff-app"></a>Exemple d’application de remise

En guise d’exemple d’utilisation de la remise dans une application Xamarin. iOS, nous avons inclus l’exemple d’application [**MonkeyBrowser**](/samples/xamarin/ios-samples/ios8-monkeybrowser) avec ce guide. L’application comporte quatre onglets que l’utilisateur peut utiliser pour naviguer sur le Web, chacun avec un type d’activité donné : météo, favori, pause café et travail.

Dans n’importe quel onglet, lorsque l’utilisateur entre une nouvelle URL et appuie sur le bouton **OK** , un nouveau `NSUserActivity` est créé pour cet onglet qui contient l’URL actuellement visitée par l’utilisateur :

[![Exemple d’application de remise](handoff-images/handoff01.png)](handoff-images/handoff01.png#lightbox)

Si l’application **MonkeyBrowser** est installée sur un autre des appareils de l’utilisateur, est connectée à iCloud à l’aide du même compte d’utilisateur, se trouve sur le même réseau et à proximité du périphérique ci-dessus, l’activité de remise s’affiche sur l’écran d’accueil (dans le coin inférieur gauche) :

[![Activité de remise affichée sur l’écran d’accueil dans le coin inférieur gauche](handoff-images/handoff02.png)](handoff-images/handoff02.png#lightbox)

Si l’utilisateur fait glisser vers le haut sur l’icône de remise, l’application est lancée et l’activité utilisateur spécifiée dans la `NSUserActivity` se poursuit sur le nouvel appareil :

[![L’activité de l’utilisateur a continué sur le nouvel appareil](handoff-images/handoff03.png)](handoff-images/handoff03.png#lightbox)

Lorsque l’activité de l’utilisateur a été correctement envoyée à un autre appareil Apple, l’appareil émetteur `NSUserActivity` reçoit un appel à la `UserActivityWasContinued`  méthode sur son `NSUserActivityDelegate` pour lui indiquer que l’activité de l’utilisateur a été transférée avec succès vers un autre appareil.
...
- [Exemple MonkeyBrowser](/samples/xamarin/ios-samples/ios8-monkeybrowser)
