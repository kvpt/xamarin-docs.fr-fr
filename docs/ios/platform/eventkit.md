...
Comme `EventStore` est comme un moteur de base de données, elle doit être de longue durée, ce qui signifie qu’elle doit être créée et détruite le moins possible au cours de la durée de vie d’une instance d’application. En fait, il est recommandé qu’une fois que vous avez créé une instance d’un `EventStore` dans une application, vous conservez cette référence pour toute la durée de vie de l’application, sauf si vous êtes sûr de ne plus en avoir besoin. en outre, tous les appels doivent être dirigés vers une seule `EventStore` instance. C’est la raison pour laquelle le modèle Singleton est recommandé pour conserver une seule instance disponible.

#### <a name="creating-an-event-store"></a>Création d’un magasin d’événements

Le code suivant illustre un moyen efficace de créer une instance unique de la `EventStore` classe et de la rendre disponible de manière statique à partir d’une application :
...
Le code ci-dessus utilise le modèle Singleton pour instancier une instance du `EventStore` lorsque l’application est chargée. Le est `EventStore` ensuite accessible globalement à partir de l’application, comme suit :
...
Avant d’être autorisé à accéder à des données via EventStore, une application doit tout d’abord demander l’accès aux données du calendrier ou aux rappels, en fonction de celui dont vous avez besoin. Pour faciliter cette opération, le `EventStore` expose une méthode appelée `RequestAccess` qui, lorsqu’elle est appelée, affiche un affichage des alertes à l’utilisateur, lui indiquant que l’application demande l’accès aux données du calendrier ou aux données de rappel, en fonction de celui qui `EKEntityType` lui est passé. Étant donné qu’il déclenche un affichage des alertes, l’appel est asynchrone et appelle un gestionnaire d’achèvement passé comme `NSAction` (ou lambda) qui reçoit deux paramètres ; une valeur booléenne indiquant si l’accès a été accordé, et un `NSError` , qui, si NOT-NULL, contient des informations d’erreur dans la demande. Par exemple, le code suivant demande l’accès aux données d’événement de calendrier et affiche un affichage des alertes si la demande n’a pas été accordée.
 ...
 *EKCalendar* représente un calendrier, qui contient un groupe d’événements de calendrier. Les calendriers peuvent être stockés dans de nombreux emplacements différents, par exemple localement, dans *icloud*, dans un emplacement de fournisseur tiers, tel qu’un *serveur Exchange* ou *Google*, etc. De nombreuses fois `EKCalendar` , permet de savoir `EventKit` où rechercher des événements ou où les enregistrer.
 ...
 *EKEvent* représente un événement de calendrier. Et `EKEvent` `EKReminder` héritent de `EKCalendarItem` et ont des champs tels que `Title` , `Notes` , et ainsi de suite.
...
Pour énumérer les calendriers que l’utilisateur a configurés sur l’appareil, appelez `GetCalendars` sur `EventStore` et transmettez le type de calendriers (rappels ou événements) que vous souhaitez recevoir :
...
Le *EKEventEditViewController* effectue beaucoup de tâches importantes pour vous si vous souhaitez créer ou modifier un événement avec la même interface utilisateur que celle présentée à l’utilisateur lors de l’utilisation de l’application de calendrier :
 ...
 [![INTERFACE utilisateur présentée à l’utilisateur lors de l’utilisation de l’application de calendrier](eventkit-images/02.png)](eventkit-images/02.png#lightbox)
...
Ensuite, pour le lancer : instanciez-le, donnez-lui une référence à, associez `EventStore` un délégué *EKEventEditViewDelegate* à ce dernier, puis affichez-le à l’aide de `PresentViewController` :
...
Pour créer un événement dans le code, utilisez la méthode de fabrique *FromStore* sur la `EKEvent` classe et définissez les données qu’il contient :
...
Pour récupérer un événement à l’aide de son ID, utilisez la méthode *EventFromIdentifier* sur `EventStore` et transmettez-lui le `EventIdentifier` qui a été extrait de l’événement :
...
Étant donné que `GetCalendarItem` retourne un `EKCalendarItem` , il doit être converti en `EKReminder` si vous avez besoin d’accéder aux données de rappel ou d’utiliser l’instance par la `EKReminder` suite.
...
Pour supprimer un événement de calendrier, appelez *RemoveEvent* sur votre `EventStore` et transmettez une référence à l’événement, et le approprié `EKSpan` :
...
Pour rechercher des événements de calendrier, vous devez créer un objet *NSPredicate* à l’aide de la méthode *PredicateForEvents* sur le `EventStore` . Un `NSPredicate` est un objet de données de requête qu’iOS utilise pour localiser les correspondances :
