...
Au cœur de Xamarin. iOS, il existe un moteur d’interopérabilité qui fait un pont entre le monde C# et le monde objective-C, ainsi que des liaisons pour les API basées sur le langage C iOS, telles que CoreGraphics et [OpenGL es](#opengles).
...
- Permet aux développeurs de sous-classer des classes objective-C :
  ...
  - Les méthodes de substitution doivent être effectuées avec le système de substitution de C#
  - Le sous-classement doit fonctionner avec les constructions C# standard
...
- Exposer les propriétés objective-C en tant que propriétés C#
...
- Encourager l’exploration dans l’IDE des API :
  ...
  - Par exemple, au lieu d’exposer un tableau faiblement typé comme suit :
    ...
    Cela donne à Visual Studio pour Mac la possibilité d’effectuer une saisie semi-automatique lors de la navigation dans l’API, rend toutes les `System.Array` opérations disponibles sur la valeur retournée et permet à la valeur de retour de participer à Linq.
...
- Prendre en charge le modèle de délégué objective-C :
  ...
  - Exposer des délégués C# (lambdas, méthodes anonymes et `System.Delegate` ) à des API objective-C en tant que blocs
...
L’espace de noms [ObjCRuntime](xref:ObjCRuntime) permet aux développeurs de relier les mondes entre C# et objective-C.
...
Xamarin. iOS reflète en C# la hiérarchie des classes de Objective-C. Par exemple, la classe de base objective-C NSObject est utilisable à partir de C# via [Foundation. NSObject](xref:Foundation.NSObject).
...
Cette opération s’effectue à l’aide de l' `Category` attribut, en spécifiant le type à étendre en tant qu’argument à l’attribut. L’exemple suivant permet d’étendre chaîne NSString.

```csharp
[Category (typeof (NSString))]
```

Chaque méthode de catégorie utilise le mécanisme normal pour exporter des méthodes vers objective-C à l’aide de l' `Export` attribut :
...
Toutes les méthodes d’extension managées doivent être statiques, mais il est possible de créer des méthodes d’instance objective-C à l’aide de la syntaxe standard pour les méthodes d’extension en C# :
...
Un scénario où cela est utile est l’ajout d’une méthode à un ensemble complet de classes dans votre code base, par exemple, cela fait que toutes les `UIViewController` instances signalent qu’elles peuvent pivoter :
...
L’espace de noms [UIKit](xref:UIKit) contient un mappage un-à-un à tous les composants d’interface utilisateur qui composent des cocoatouch sous la forme de classes C#. L’API a été modifiée pour suivre les conventions utilisées dans le langage C#.

Les délégués C# sont fournis pour les opérations courantes. Pour plus d’informations, consultez la section [délégués](#delegates) .
...
Xamarin. iOS n’est pas simplement une liaison à la plateforme objective-C sous-jacente. Il étend le système de type .NET et le système de distribution pour mieux mélanger C# et objective-C.

Tout comme P/Invoke est un outil utile pour appeler des bibliothèques natives sur Windows et Linux, ou lorsque la prise en charge de IJW peut être utilisée pour COM Interop sur Windows, Xamarin. iOS étend le runtime pour prendre en charge la liaison d’objets C# aux objets objective-C.
...
Lorsqu’il y a un sens, les types C# sont exposés au lieu des types de fondation de bas niveau, à l’univers C#.  Cela signifie que [l’API utilise le type c# « String » au lieu de chaîne NSString](~/ios/internals/api-design/nsstring.md) et qu’il utilise des tableaux c# fortement typés au lieu d’exposer NSArray.

En général, dans la conception Xamarin. iOS et Xamarin. Mac, l’objet sous-jacent `NSArray` n’est pas exposé. Au lieu de cela, le Runtime convertit automatiquement les `NSArray` s en tableaux fortement typés d’une `NSObject` classe. Ainsi, Xamarin. iOS n’expose pas une méthode faiblement typée telle que GetViews pour retourner un NSArray :
...
Il existe quelques méthodes exposées dans `NSArray` , pour les cas où vous voudrez peut-être utiliser un `NSArray` directement, mais leur utilisation est déconseillée dans la liaison d’API.

En outre, dans la **API classique** au lieu d’exposer `CGRect` , `CGPoint` et `CGSize` à partir de l’API coregraphics, nous avons remplacé ceux-ci par les `System.Drawing` implémentations `RectangleF` , `PointF` et `SizeF` comme ils aideront les développeurs à conserver le code OpenGL existant qui utilise OpenTK. Lors de l’utilisation de la nouvelle **API unifiée**64 bits, l’API coregraphics doit être utilisée.

#### <a name="inheritance"></a>Héritage

La conception de l’API Xamarin. iOS permet aux développeurs d’étendre les types objective-C natifs de la même façon qu’ils étendent un type C#, en utilisant le mot clé « override » sur une classe dérivée, ainsi que le chaînage de l’implémentation de base à l’aide du mot clé C# « base ».
...
Quand vous créez des classes .NET qui sont des instances de types créées par Interface Builder, vous devez fournir un constructeur qui accepte un `IntPtr` paramètre unique.
...
Objective-C et C# ont des significations différentes pour le mot délégué dans chaque langage.

Dans le monde objective-C, et dans la documentation que vous trouverez en ligne sur CocoaTouch, un délégué est généralement une instance d’une classe qui répond à un ensemble de méthodes. Cela est très similaire à une interface C#, à la différence que les méthodes ne sont pas toujours obligatoires.
...
- Pour fournir des notifications à votre code (semblable à la remise d’événements en C# ou GTK +).
- Pour implémenter des modèles pour les contrôles de visualisation de données.
- Pour piloter le comportement d’un contrôle.

Le modèle de programmation a été conçu pour réduire la création de classes dérivées afin de modifier le comportement d’un contrôle. Cette solution est similaire à celle des autres kits d’outils GUI effectués au fil des années : les signaux de GTK, les emplacements QT, les événements WinForms, les événements WPF/Silverlight, etc. Pour éviter d’avoir des centaines d’interfaces (une pour chaque action) ou de demander aux développeurs d’implémenter un trop grand nombre de méthodes dont ils n’ont pas besoin, Objective-C prend en charge les définitions de méthode facultatives. Cela est différent des interfaces C# qui requièrent que toutes les méthodes soient implémentées.

Dans les classes objective-C, vous verrez que les classes qui utilisent ce modèle de programmation exposent une propriété, généralement appelée `delegate` , qui est requise pour implémenter les parties obligatoires de l’interface et zéro, ou plus, des parties facultatives.
...
2. [Fortement typé via une `Delegate` propriété](#strongly-typed-via-a-delegate-property)
3. [Faiblement typé à l’aide d’une `WeakDelegate` propriété](#loosely-typed-via-the-weakdelegate-property)
...
Ce modèle est également utilisé pour contrôler le comportement de certains contrôles, par exemple dans le cas UIWebView, la propriété [UIWebView. ShouldStartLoad](xref:UIKit.UIWebView.ShouldStartLoad) permet `UIWebView` à l’instance de contrôler si le `UIWebView` charge une page ou non.
...
Partout où une propriété fortement typée `Delegate` est exposée dans la liaison de Xamarin. iOS, une `WeakDelegate` propriété correspondante est également exposée.

Lorsque vous utilisez `WeakDelegate` , vous êtes chargé de décorer correctement votre classe à l’aide de l’attribut [Export](xref:Foundation.ExportAttribute) pour spécifier le sélecteur. Par exemple :
...
Notez qu’une fois que la `WeakDelegate` propriété a été assignée, la propriété n’est `Delegate` pas utilisée. En outre, si vous implémentez la méthode dans une classe de base héritée que vous souhaitez [Exporter], vous devez en faire une méthode publique.
...
Cela indique au langage de créer et de construire une instance de la classe « SomethingDelegate » et d’assigner la valeur à la propriété de délégué sur la variable foo. Ce mécanisme est pris en charge par Xamarin. iOS et C#. la syntaxe est la suivante :
...
### <a name="mapping-delegates-to-c"></a>Mappage de délégués à C\#
...
Le deuxième formulaire est de fournir une notification pour les événements. Dans ce cas, bien que nous exposons toujours l’API sous la forme décrite ci-dessus, nous fournissons également des événements C#, qui doivent être plus simples à utiliser pour les opérations rapides et intégrés aux délégués anonymes et aux expressions lambda en C#.
...
Comme indiqué précédemment, Xamarin. iOS prend en charge le modèle de programmation basé sur les événements C# et le modèle objective-C délégué, où vous pouvez créer une nouvelle classe qui implémente le délégué et substitue les méthodes souhaitées.
...
Par exemple, si vous souhaitez que votre classe réponde au `UITextFieldDelegate.textFieldShouldClear` message : et à `UIWebViewDelegate.webViewDidStartLoad` : dans la même instance d’une classe, vous devez utiliser la déclaration d’attribut [Export] :
...
Les noms C# pour les méthodes ne sont pas importants ; toutes les choses importantes sont les chaînes passées à l’attribut [Export].

Quand vous utilisez ce style de programmation, assurez-vous que les paramètres C# correspondent aux types réels que le moteur d’exécution passera.
...
Le nom du sélecteur objective-C (« applicationDidFinishLaunching : ») est déclaré avec l’attribut Export et la classe est inscrite avec l' `[Register]` attribut.
...
Les avantages sont qu’il n’est pas nécessaire de plonger dans les fichiers d’en-tête objective-C pour rechercher le sélecteur, les types des arguments ou le mappage à C#, et que vous obtenez IntelliSense à partir de Visual Studio pour Mac, ainsi que les types forts
...
>
> [![Saisie d’un nom d’élément dans le concepteur iOS](images/designeroutlet.png)](images/designeroutlet.png#lightbox)
>
...
Il s’agit d’une description de bas niveau de la façon dont les sorties s’intègrent à C# et est fournie pour les utilisateurs expérimentés de Xamarin. iOS. Lorsque vous utilisez Visual Studio pour Mac le mappage est effectué automatiquement en arrière-plan à l’aide du code généré sur le vol pour vous.
...
Cela charge l’interface utilisateur à partir du bec. À présent, pour accéder aux prises, il est nécessaire d’indiquer au runtime que nous souhaitons y accéder. Pour ce faire, la `UIViewController` sous-classe doit déclarer les propriétés et les annoter avec l’attribut [Connect]. Comme ceci :
...
La création de nouveaux sélecteurs en C# est très simple : il vous suffit de créer une nouvelle instance de la `ObjCRuntime.Selector` classe et d’utiliser le résultat à n’importe quel endroit de l’API qui l’exige. Par exemple :

```csharp
var selector_add = new Selector ("add:plus:");
```

Pour qu’une méthode C# réponde à un appel de sélecteur, elle doit hériter du `NSObject` type et la méthode c# doit être décorée avec le nom du sélecteur à l’aide de l' `[Export]` attribut. Par exemple :
...
Il s’agit du constructeur par défaut pour une classe, et dans les classes fournies par Xamarin. iOS, cela initialise la classe Foundation. NSObject et toutes les classes comprises entre, et à la fin, la chaîne à la méthode objective-C `init` sur la classe.
...
La conception de l’API Xamarin. iOS ne déclenche pas d’exceptions objective-C comme des exceptions C#. La conception s’impose qu’aucun garbage collection ne soit envoyé au monde objective-C en premier lieu et que toutes les exceptions qui doivent être produites sont produites par la liaison elle-même avant que des données non valides ne soient passées au monde objective-C.

#### <a name="notifications"></a>Notifications

Dans iOS et OS X, les développeurs peuvent s’abonner à des notifications diffusées par la plateforme sous-jacente. Pour ce faire, utilisez la `NSNotificationCenter.DefaultCenter.AddObserver` méthode. La `AddObserver` méthode accepte deux paramètres : l’un est la notification à laquelle vous souhaitez vous abonner ; l’autre est la méthode à appeler lorsque la notification est déclenchée.
...
Xamarin. iOS dispose d’un garbage collector qui prend en charge la libération des ressources lorsque celles-ci ne sont plus utilisées. Outre le garbage collector, tous les objets qui dérivent de `NSObject` implémentent l' `System.IDisposable` interface.
...
Toutefois, la suppression d’un objet en C# ne signifie pas que l’objet sera nécessairement détruit. Il vous suffit de libérer la référence que C# avait pour l’objet. Il est possible que l’environnement de cacao ait gardé une référence pour sa propre utilisation. Par exemple, si vous affectez une image à la propriété image d’un UIImageView, puis que vous supprimez l’image, le UIImageView sous-jacent a pris sa propre référence et conserve une référence à cet objet jusqu’à ce qu’il ait terminé de l’utiliser.
...
En interne, lorsque mono crée [des références chaîne NSString à partir de chaînes C#](~/ios/internals/api-design/nsstring.md), il les supprime immédiatement pour réduire la quantité de travail que le garbage collector a à faire. Moins il y a d’objets à gérer, plus le GC s’exécutera rapidement.
