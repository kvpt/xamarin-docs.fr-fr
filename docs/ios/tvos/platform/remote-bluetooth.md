...
[![Le contrôleur de jeu et distant Bluetooth](remote-bluetooth-images/intro01.png)](remote-bluetooth-images/intro01.png#lightbox)
...
[![Siri à distance](remote-bluetooth-images/remote01.png)](remote-bluetooth-images/remote01.png#lightbox)
...
|**Page d'accueil**<br />Appuyez sur pour revenir à l’écran d’accueil, double-cliquez pour afficher les applications en cours d’exécution, appuyez sur la touche et maintenez-la enfoncée.|n/a|n/a|
...
En outre, les enfonceurs de bouton de menu peuvent être détectés à l’aide d’un module de reconnaissance de mouvement avec des `UIKit` éléments standard. Si vous interceptez le bouton de menu enfoncé, vous êtes chargé de fermer l’affichage et le contrôleur d’affichage en cours et de revenir au précédent.
...
1. Dans la **Explorateur de solutions**, double-cliquez sur le `Main.storyboard` fichier et ouvrez-le pour modifier le concepteur d’interface.
2. Faites glisser un module de **reconnaissance de mouvement TAP** de la **bibliothèque** et déposez-le sur la vue :

    [![Module de reconnaissance de mouvement TAP](remote-bluetooth-images/storyboard01.png)](remote-bluetooth-images/storyboard01.png#lightbox)
3. Cochez la case **Sélectionner** dans la section du **bouton** de l' **inspecteur d’attribut**:

    [![Cocher sélectionner](remote-bluetooth-images/storyboard02.png)](remote-bluetooth-images/storyboard02.png#lightbox)
...
5. Ensuite, associez une **action** à partir du module de **reconnaissance de mouvement TAP** et appelez-la `TouchSurfaceClicked` :

    [![Action du module de reconnaissance de mouvement TAP](remote-bluetooth-images/storyboard03.png)](remote-bluetooth-images/storyboard03.png#lightbox)
6. Enregistrez vos modifications et revenez à Visual Studio pour Mac.

Modifiez le fichier de votre contrôleur d’affichage (exemple `FirstViewController.cs` ) et ajoutez le code suivant pour gérer le mouvement déclenché :
...
Si vous le souhaitez, vous pouvez créer des mouvements directement dans le code C# et les ajouter aux vues dans votre interface utilisateur. Par exemple, pour ajouter une série de modules de reconnaissance de mouvement de balayage, modifiez votre contrôleur d’affichage et ajoutez le code suivant :
...
Si vous créez un type personnalisé basé sur `UIKit` dans votre application Xamarin. tvOS (par exemple `UIView` ), vous avez également la possibilité de fournir une gestion de bas niveau de l’appui sur les boutons via des `UIPress` événements.

Un `UIPress` événement consiste à tvOS ce qu' `UITouch` est un événement iOS, à l’exception des `UIPress` informations relatives aux presses sur les Siri distants ou autres périphériques Bluetooth attachés (comme un contrôleur de jeu). `UIPress`les événements décrivent le bouton enfoncé et son état (commencé, annulé, modifié ou terminé).

Pour les boutons analogiques sur des appareils tels que les contrôleurs de jeu Bluetooth, `UIPress` retourne également la quantité de force appliquée au bouton. La `Type` propriété de l' `UIPress` événement définit le bouton physique qui a changé d’État, tandis que les autres propriétés décrivent la modification qui s’est produite.
...
Comme `UITouch` pour les événements, si vous devez implémenter l’un des `UIPress` remplacements d’événements, vous devez implémenter les quatre.
...
[![Contrôleurs de jeu Bluetooth](remote-bluetooth-images/game01.png)](remote-bluetooth-images/game01.png#lightbox)
...
- Le profil de contrôleur de jeu micro ( `GCMicroGamepad` ) a été ajouté pour cibler le Siri distant.
...
Pour activer la prise en charge du contrôleur de jeu dans votre application Xamarin. tvOS, double-cliquez sur le `Info.plist` fichier dans le **Explorateur de solutions** pour l’ouvrir et le modifier :

[![Éditeur info. plist](remote-bluetooth-images/game02.png)](remote-bluetooth-images/game02.png#lightbox)
...
La télécommande Siri fournie avec Apple TV peut être utilisée en tant que contrôleur de jeu limité. Comme les autres contrôleurs de jeu, il s’affiche dans la structure du contrôleur de jeu en tant qu' `GCController` objet et prend en charge à la fois les `GCMotion` `GCMicroGamepad` profils et.
...
Contrairement à iOS où les événements de contrôleur de jeu peuvent être reçus en parallèle avec les événements tactiles, tvOS traite tous les événements de bas niveau pour fournir des événements de haut niveau `UIKit` . Par conséquent, si vous avez besoin d’accéder aux événements du contrôleur de jeu de bas niveau, vous devez désactiver le `UIKit` comportement par défaut de.

Sur tvOS, lorsque vous souhaitez traiter directement une entrée de contrôleur de jeu, vous devez utiliser une `GCEventViewController` (ou une sous-classe) pour afficher l’interface utilisateur du jeu. Chaque fois qu’un `GCEventViewController` est le *premier répondeur*, l’entrée du contrôleur de jeu est capturée et remise à votre application par le biais de l’infrastructure du contrôleur de jeu.
