...
ms.locfileid: "73030491"
---
# <a name="working-with-tvos-buttons-in-xamarin"></a>Utilisation des boutons tvOS dans Xamarin

Utilisez une instance de la classe `UIButton` pour créer un bouton pouvant être sélectionné dans une fenêtre tvOS. Quand l’utilisateur sélectionne un bouton, il envoie un message d’action à l’objet cible, ce qui permet à votre application Xamarin. tvOS de répondre à l’entrée de l’utilisateur.
...
1. Vous serez automatiquement dirigé vers la vue `ViewController.cs` dans laquelle vous pouvez placer la nouvelle action dans votre code à l’aide des touches de direction **haut** et **bas** : 
...
Tant que la propriété `Enabled` d’un bouton est `true` et qu’elle n’est pas couverte par un autre contrôle ou vue, elle peut être rendue en élément actif à l’aide de la Siri distante. Si l’utilisateur sélectionne le bouton et clique sur la surface tactile, l’action `ButtonPressed` définie ci-dessus est exécutée.
...
Si vous le souhaitez, vous pouvez créer un C# `UIButton` dans le code et l’ajouter à la vue de l’application tvOS. Exemple :
...
Lorsque vous créez un `UIButton` dans le code, vous spécifiez son `UIButtonType` comme l’un des éléments suivants :
...
Ensuite, définissez le titre du bouton. `UIButtons` sont différents des contrôles `UIKit` dans la mesure où ils ont un État pour que vous ne puissiez pas simplement modifier le titre, vous devez le modifier pour un `UIControlState`donné. Exemple :

```csharp
button.SetTitle ("Hello", UIControlState.Normal);
```

Ensuite, utilisez l’événement `AllEvents` pour voir à quel moment l’utilisateur a cliqué sur le bouton. Exemple :
...
> [!IMPORTANT]
> Bien qu’il soit possible d’assigner des actions telles que des `TouchUpInside` à un `UIButton`, il ne sera jamais appelé, car Apple TV n’a pas d’écran tactile ou ne prend pas en charge les événements tactiles. Vous devez toujours utiliser des événements tels que **AllEvents** ou **PrimaryActionTriggered**.

<a name="Styling-a-Button" />

## <a name="styling-a-button"></a>Stylisation d’un bouton

tvOS fournit plusieurs propriétés d’une `UIButton` qui peuvent être utilisées pour fournir son titre et lui appliquer un style avec des éléments tels que la couleur et les images d’arrière-plan.

<a name="Button-Titles" />

### <a name="button-titles"></a>Titres de bouton

Comme nous l’avons vu plus haut, les `UIButtons` sont différentes de la plupart des contrôles de `UIKit` dans la mesure où ils ont un État pour que vous ne puissiez pas simplement modifier le titre, vous devez le modifier pour un `UIControlState`donné. Exemple :

```csharp
button.SetTitle ("Hello", UIControlState.Normal);
```

Vous pouvez définir la couleur du titre du bouton à l’aide de la méthode `SetTitleColor`. Exemple :

```csharp
button.SetTitleColor (UIColor.White, UIControlState.Normal);
```

Vous pouvez ajuster l’ombre du titre à l’aide de la `SetTitleShadowColor`. Exemple :
...
En outre, vous pouvez utiliser le texte avec attributs comme titre du bouton. Exemple :
...
Une `UIButton` peut être attachée à une image et peut utiliser une image comme arrière-plan.

Pour définir l’image d’arrière-plan d’un bouton pour un `UIControlState`donné, utilisez le code suivant :

```csharp
button.SetBackgroundImage(UIImage.FromFile("my image.png"), UIControlState.Normal);
```

Affectez à la propriété `AdjustsImageWhenHiglighted` la valeur `true` pour dessiner l’image plus claire lorsque le bouton est mis en surbrillance (il s’agit de la valeur par défaut). Affectez à la propriété `AdjustsImageWhenDisabled` la valeur `true` pour dessiner l’image plus sombre lorsque le bouton est désactivé (là encore, il s’agit de la valeur par défaut).
...
Utilisez la propriété `TintColor` pour définir une teinte de couleur appliquée au titre et à l’image du bouton. Pour les boutons du type `Custom`, cette propriété n’a aucun effet, vous devez implémenter le comportement `TintColor` vous-même.
