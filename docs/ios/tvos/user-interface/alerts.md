...
Si vous avez besoin d’attirer l’attention de l’utilisateur tvOS ou de demander l’autorisation d’effectuer une action destructrice (telle que la suppression d’un fichier), vous pouvez présenter un message d’alerte à l’aide des `UIAlertViewController` éléments suivants :

[![Exemple de UIAlertViewController](alerts-images/alert01.png)](alerts-images/alert01.png#lightbox)
...
[![Un exemple d’alerte](alerts-images/alert04.png)](alerts-images/alert04.png#lightbox)
...
- **Marquez clairement les boutons destructifs** : pour les boutons qui exécutent une action destructrice (telle que la suppression d’un fichier), marquez-les clairement avec le `UIAlertActionStyle.Destructive` style.
...
La `AddAction` méthode ajoute l’action donnée au `UIAlertViewController` et, enfin, la `PresentViewController (alertController, true, null)` méthode affiche l’alerte donnée à l’utilisateur.
...
[![Champ de texte dans une alerte](alerts-images/alert02.png)](alerts-images/alert02.png#lightbox)
...
[![Entrée de texte](alerts-images/alert03.png)](alerts-images/alert03.png#lightbox)
...
Une fois que l’utilisateur a entré une valeur dans le champ de texte, nous pouvons utiliser la `field` variable pour accéder à cette valeur.
...
Étant donné que l’affichage d’alertes simples et courantes à l’aide de `UIAlertViewController` peut entraîner un certain nombre de code en double, vous pouvez utiliser une classe d’assistance pour réduire la quantité de code répétitif. Par exemple :
