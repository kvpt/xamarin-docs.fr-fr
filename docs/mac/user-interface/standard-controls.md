...
- **Bouton de texture ronde** -un `NSButton` avec un style de `NSTexturedRoundedBezelStyle`.
- **Contrôle segmenté arrondi texturé** : `NSSegmentedControl` avec un style de `NSSegmentStyleTexturedRounded`.
- **Contrôle segmenté arrondi texturé** : `NSSegmentedControl` avec un style de `NSSegmentStyleSeparated`.
- **Menu contextuel avec texture ronde** : `NSPopUpButton` avec un style de `NSTexturedRoundedBezelStyle`.
- **Menu déroulant avec texture ronde** : `NSPopUpButton` avec un style de `NSTexturedRoundedBezelStyle`.
...
Spécifie que le contrôle sélectionné (dans l' **affichage des hiérarchies** & éditeur de l' **interface**) sera bloqué sur l’emplacement supérieur et droit de la fenêtre ou de la vue, car elle est redimensionnée ou déplacée. 
...
Où l’instruction `[Register("SourceListView")]` expose la classe `SourceListView` à Objective-C afin que puisse être utilisé dans Interface Builder. Pour plus d’informations, reportez-vous à la section [exposition des C# classes/méthodes à Objective-C](~/mac/internals/how-it-works.md) du document [Xamarin. Mac Internals.](~/mac/internals/how-it-works.md) elle explique les commandes`Register`et`Export`utilisées C# pour relier vos classes à objective-c. objets et éléments d’interface utilisateur.

Avec le code ci-dessus en place, vous pouvez faire glisser un contrôle AppKit du type de base que vous étendez sur l’aire de conception (dans l’exemple ci-dessous, une **liste source**), basculer vers l' **inspecteur d’identité** et définir la **classe personnalisée** sur le nom vous avez exposé à Objective-C (exemple `SourceListView`) :
...
Pour qu’un contrôle AppKit soit accessible dans C# le code, il doit être exposé en tant que **sortie** ou **action**. Pour ce faire, sélectionnez le contrôle donné dans la **hiérarchie d’interface** ou dans l' **éditeur d’interface** et basculez vers la **vue Assistant** (assurez-vous que la `.h` de votre fenêtre est sélectionnée pour modification) :

[![](standard-controls-images/edit11.png "Selecting the correct file to edit")](standard-controls-images/edit11.png#lightbox)

Contrôle : faites glisser le contrôle AppKit sur le fichier `.h` pour commencer à créer une **sortie** ou une **action**:
...
Si vous sélectionnez la `SplitViewController.designer.cs` dans le **Explorateur de solutions** vous pouvez voir comment votre **sortie** et votre **action** ont été reliées dans notre C# code :

[![](standard-controls-images/sync01.png "Synchronizing Changes with Xcode")](standard-controls-images/sync01.png#lightbox)

Notez comment la définition dans le fichier `SplitViewController.designer.cs` :
...
Ligner avec la définition dans le fichier `MainWindow.h` dans Xcode :
...
Comme vous pouvez le voir, Visual Studio pour Mac écoute les modifications apportées au fichier `.h`, puis synchronise automatiquement ces modifications dans le fichier `.designer.cs` respectif pour les exposer à votre application. Vous pouvez également remarquer que `SplitViewController.designer.cs` est une classe partielle, de sorte que Visual Studio pour Mac n’a pas à modifier `SplitViewController.cs` qui remplacerait les modifications que nous avons apportées à la classe.

Normalement, vous n’avez pas besoin d’ouvrir la `SplitViewController.designer.cs` vous-même, elle a été présentée ici à titre éducatif uniquement.
...
Pour les boutons qui ont été exposés via des **actions**, une méthode `public partial` sera automatiquement créée pour vous avec le nom que vous avez choisi dans Xcode. Pour répondre à l' **action**, terminez la méthode partielle de la classe sur laquelle l' **action** a été définie. Exemple :
...
Pour les boutons qui ont un État ( **par exemple, activé ou** **désactivé**), l’État peut être vérifié ou défini avec la propriété `State` sur l’énumération `NSCellStateValue`. Exemple :
...
Les cases à cocher et les cases d’option (exposées via **des** **prises**) ont un État (par exemple, activé ou **désactivé**), l’État peut être vérifié ou défini avec la propriété `State` sur l’énumération `NSCellStateValue`. Exemple :
...
Pour sélectionner un bouton dans un groupe de cases d’option, exposez la case d’option pour sélectionner comme **sortie** et définissez sa propriété `State`. Par exemple :
...
Vous pouvez utiliser la propriété `Tag` pour voir quelle case d’option a été sélectionnée.
...
Pour utiliser une source de données externe, vous allez créer une instance de la source de données du contrôle Menu (`NSComboBoxDataSource` par exemple) et substituer plusieurs méthodes pour fournir les données nécessaires :

- `ItemCount` : retourne le nombre d’éléments dans la liste.
- `ObjectValueForItem` : renvoie la valeur de l’élément pour un index donné.
...
- `IntercellSpacing`-ajuster la quantité d’espace autour d’un élément donné en fournissant un `NSSize` où le `Width` spécifie les marges gauche et droite et le `Height` spécifie l’espace avant et après un élément.
- `ItemHeight` : spécifie la hauteur de chaque élément de la liste.

Pour les types de `NSPopupButtons`déroulants, le premier élément de menu fournit le titre du contrôle. Par exemple : 
...
- `SelectedIndex` : renvoie l’index de l’élément actuellement sélectionné.
- `SelectedValue`-retourne la valeur de l’élément actuellement sélectionné.

Utilisez la `ScrollItemAtIndexToTop` pour présenter l’élément à l’index donné en haut de la liste et la `ScrollItemAtIndexToVisible` pour faire défiler jusqu’à la liste jusqu’à ce que l’élément à l’index spécifié soit visible.
...
Pour les contrôles `NSComboBox`, ils incluent tous les mêmes événements que le `NSTextField`, tel que l’événement `Changed` qui est appelé chaque fois que l’utilisateur modifie la valeur du texte dans la zone de liste déroulante.
...
Ou en attachant un **délégué** à l’événement `Activated`. Exemple :
...
Pour définir ou lire la valeur d’un contrôle Selection, utilisez la propriété `IntValue`. Exemple :
...
L' `NSDatePicker` possède les propriétés suivantes pour travailler directement avec la date et l’heure :
...
- **TimeZone** : fuseau horaire de l’utilisateur en tant que `NSTimeZone`.
...
Il existe deux façons de suivre le moment où un contrôle indicateur a une interaction avec l’utilisateur, soit en l’exposant en tant qu' **action** **, soit** en joignant un **délégué** à l’événement `Activated`. Exemple :
...
Pour lire ou définir la valeur du contrôle d’indicateur, utilisez la propriété `DoubleValue`. Exemple :

```csharp
FeedbackLabel.StringValue = string.Format("Rating: {0:###}",Rating.DoubleValue);
```

Les indicateurs de progression indéterminés et asynchrones doivent être animés lorsqu’ils sont affichés. Utilisez la méthode `StartAnimation` pour démarrer l’animation quand elles sont affichées. Exemple :
...
L’appel de la méthode `StopAnimation` arrêtera l’animation.
...
Utilisez la propriété `StringValue` pour lire ou définir la valeur du champ. Exemple :

```csharp
FeedbackLabel.StringValue = string.Format("User ID: {0}",UserField.StringValue);
```

Pour les champs qui affichent ou modifient des valeurs numériques, vous pouvez utiliser la propriété `IntValue`. Exemple :

```csharp
FeedbackLabel.StringValue = string.Format("Number: {0}",NumberField.IntValue);
```

Une `NSTextView` fournit une zone d’affichage et une modification de texte complète avec une mise en forme intégrée. Comme un `NSTextField`, utilisez la propriété `StringValue` pour lire ou définir la valeur de la zone.
...
1. Ouvrez le fichier `.storyboard` de la fenêtre dans laquelle vous souhaitez ajouter un menu segue en double-cliquant dessus dans le **Explorateur de solutions**
