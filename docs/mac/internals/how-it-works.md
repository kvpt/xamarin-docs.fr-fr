...
Dans Xamarin. Mac, une application fait un pont entre deux mondes : le runtime objective-C contient des instances de classes natives (`NSString`, `NSApplication`, etc.) C# et le Runtime contient des instances de classes managées (`System.String`, `HttpClient`, etc.) . Entre ces deux mondes, Xamarin. Mac crée un pont bidirectionnel afin qu’une application puisse appeler des méthodes (sélecteurs) en Objective-C (par exemple `NSApplication.Init`) et objective-C peut appeler les C# méthodes de l’application (comme les méthodes sur un délégué d’application). En général, les appels à Objective-C sont gérés de façon transparente par le biais des **P/Invoke** et du code d’exécution fourni par Xamarin.

<a name="exposing-classes" />

## <a name="exposing-c-classes--methods-to-objective-c"></a>Exposition de C# classes/méthodes à Objective-C

Toutefois, pour objective-C à rappeler dans les objets d' C# une application, ils doivent être exposés de manière à ce que objective-c puisse comprendre. Cette opération s’effectue via les attributs `Register` et `Export`. Prenons l’exemple suivant :
...
Dans la plupart des cas, il s’agit d’un détail d’implémentation que le développeur peut ignorer, car la plupart des rappels qu’une application reçoit sont soit via des méthodes substituées sur des classes de `base` (telles que `AppDelegate`, `Delegates`, `DataSources`), soit sur des **actions** passées dans des API. Dans tous ces cas, les attributs de `Export` ne sont pas nécessaires C# dans le code.
...
En général, le développeur doit conserver les constructeurs `IntPtr` et `NSCoder` générés lors de la création de certains types, tels que les `NSViews` personnalisés. Si Xamarin. Mac doit appeler l’un de ces constructeurs en réponse à une demande d’exécution objective-C et que vous l’avez supprimée, l’application se bloque dans le code natif et il peut être difficile de déterminer exactement le problème.
...
- **Amélioration des journaux des pannes « natifs** » : si une application Xamarin. Mac se bloque en code natif, ce qui est souvent le cas lors de l’exécution d’appels non valides dans des API de cacao (comme l’envoi d’un `null` dans une méthode qui ne l’accepte pas), les journaux de défaillance natifs avec des frames JIT sont difficile à analyser. Étant donné que les frames JIT n’ont pas d’informations de débogage, il y aura plusieurs lignes avec des décalages hexadécimaux et aucune indication sur ce qui se passait. L’AOA génère des frames nommés « réels » et les suivis sont beaucoup plus faciles à lire. Cela signifie également que l’application Xamarin. Mac interagira mieux avec les outils natifs tels que **lldb** et **instruments**.
...
L’AOA est activé dans Xamarin. Mac en double-cliquant sur le **nom du projet** dans le **Explorateur de solutions**, en accédant à la **Build Mac** et en ajoutant des `--aot:[options]` au champ **MMP arguments supplémentaires :** (où `[options]` est une ou plusieurs options à contrôler le type AOA, voir ci-dessous). Exemple :
...
- `all`-AOT compile chaque assembly dans le monofaisceau.
- `core`-AOT compile les assemblys `Xamarin.Mac`, `System` et `mscorlib`.
- `sdk`-AOT compile les assemblys de `Xamarin.Mac` et des bibliothèques de classes de base (BCL).
...
Par exemple, `--aot:all,-MyAssembly.dll` activerait la compilation AOA sur tous les assemblys du monogroupe, _à l’exception_ de `MyAssembly.dll` et `--aot:core|hybrid,+MyOtherAssembly.dll,-mscorlib.dll` activer Hybrid, l’AOA de code inclut le `MyOtherAssembly.dll` et l’exclusion de la `mscorlib.dll`.
...
Dans le cadre d’une application Xamarin. Mac, l’infrastructure de cacao est basée sur Apple et le runtime objective-C. La création d’un pont entre ce « monde natif » et le « monde géré C# » de est la responsabilité principale de Xamarin. Mac. Une partie de cette tâche est gérée par le Bureau d’enregistrement, qui est exécuté dans `NSApplication.Init ()` méthode. C’est l’une des raisons pour lesquelles toute utilisation des API de cacao dans Xamarin. Mac requiert l’appel d’abord `NSApplication.Init`.

La tâche du Bureau d’enregistrement est d’informer le runtime objective-C de l’existence des C# classes de l’application qui dérivent de classes telles que `NSApplicationDelegate`, `NSView`, `NSWindow`et `NSObject`. Cela nécessite une analyse de tous les types dans l’application pour déterminer ce qui doit être inscrit et les éléments de chaque type à signaler.
...
L’inscription statique partielle, introduite pour la première fois dans Xamarin. iOS 8,13, offre au développeur le meilleur des deux options. En précalculant les informations d’enregistrement de chaque élément dans `Xamarin.Mac.dll` et en envoyant ces informations avec Xamarin. Mac dans une bibliothèque statique (qui ne doit être liée qu’au moment de la génération), Microsoft a supprimé la majeure partie du temps de réflexion du Bureau d’enregistrement dynamique. sans affecter le temps de génération.

### <a name="enabling-the-partial-static-registrar"></a>Activation du Registre statique partiel

L’inscription statique partielle est activée dans Xamarin. Mac en double-cliquant sur **le nom du projet** dans le **Explorateur de solutions**, en accédant à la **Build Mac** et en ajoutant des `--registrar:static` au champ **arguments MMP supplémentaires :** . Exemple :
...
- [À propos de `newrefcount`](~/ios/internals/newrefcount.md)
