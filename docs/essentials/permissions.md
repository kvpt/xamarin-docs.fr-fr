...
Pour demander une autorisation aux utilisateurs, utilisez la méthode `RequestAsync`, ainsi que l’autorisation spécifique à demander. Si l’utilisateur a précédemment accordé une autorisation et qu’il ne l’a pas révoquée, cette méthode retourne `Granted` immediatelly et n’affiche pas de boîte de dialogue. 
...
Notez que sur certaines plateformes, une demande d’autorisation ne peut être activée qu’une seule fois. Les invites supplémentaires doivent être gérées par le développeur pour vérifier si une valeur d’État est dans l’État `Denied` et demander à l’utilisateur de l’activer manuellement.
...
Si une autorisation est marquée comme ![non prise en charge](~/media/shared/no.png "non pris en charge") , elle retourne toujours `Granted` lorsqu’elle est activée ou demandée.
...
Lors de l’implémentation d’une autorisation sur une plateforme spécifique, la classe `BasePlatformPermission` peut être héritée de. Cela fournit des méthodes d’assistance de plateforme supplémentaires pour vérifier automatiquement les déclarations.
