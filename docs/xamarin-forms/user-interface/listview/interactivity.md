...
description: Cet article explique comment ajouter une interactivité à un ListView Xamarin. Forms en implémentant des sélections, des actions de contexte et une extraction à actualiser.
ms.prod: xamarin
...
Les actions de contexte peuvent être C# implémentées dans et XAML. Vous trouverez ci-dessous des guides spécifiques pour les deux, mais commençons par examiner certains détails d’implémentation clés pour les deux.
...
`MenuItem` éléments peuvent être créés dans une collection XAML. Le code XAML ci-dessous illustre une cellule personnalisée avec deux actions de contexte implémentées :
...
Les actions de contexte peuvent être implémentées dans n’importe quelle sous-classe `Cell` (tant qu’elles ne sont pas utilisées comme en-tête de groupe) en créant des instances `MenuItem` et en les ajoutant à la collection `ContextActions` pour la cellule. Vous pouvez configurer les propriétés suivantes pour l’action de contexte :
...
Les utilisateurs s’attendent à ce que l’extraction d’une liste de données actualise cette liste. Le contrôle de [`ListView`](xref:Xamarin.Forms.ListView) prend en charge ce qui est prêt à l’emploi. Pour activer la fonctionnalité d’extraction vers l’actualisation, définissez [`IsPullToRefreshEnabled`](xref:Xamarin.Forms.ListView.IsPullToRefreshEnabled) sur `true` :
