...
Ces propriétés, à l’exception des propriétés `MapElements`, `Pins`et `VisibleRegion` , sont sauvegardées par [`BindableProperty`](xref:Xamarin.Forms.BindableProperty) des objets, ce qui signifie qu’elles peuvent être des cibles de liaisons de données.

La [`Map`](xref:Xamarin.Forms.Maps.Map) classe définit également un `MapClicked` événement qui est déclenché lorsque le mappage est frappé. L' `MapClickedEventArgs` objet qui accompagne l’événement a une propriété unique nommée `Position`, de type. [`Position`](xref:Xamarin.Forms.Maps.Position) Lorsque l’événement est déclenché, la `Position` propriété est définie sur l’emplacement de la carte qui a été frappé. Pour plus d’informations [`Position`](xref:Xamarin.Forms.Maps.Position) sur la structure, consultez [position et distance](position-distance.md)de la carte.

Pour plus d’informations [`ItemsSource`](xref:Xamarin.Forms.Maps.Map.ItemsSource)sur [`ItemTemplate`](xref:Xamarin.Forms.Maps.Map.ItemTemplate)les propriétés `ItemTemplateSelector` , et, consultez [afficher une collection de codes confidentiels](pins.md#display-a-pin-collection).
...
> [!NOTE]
> Une définition `xmlns` d’espace de noms supplémentaire est requise pour référencer les contrôles Xamarin. Forms. Maps. Dans l’exemple précédent, `Xamarin.Forms.Maps` l’espace de noms est référencé `maps` via le mot clé.
...
Cet exemple appelle le constructeur [`Map`](xref:Xamarin.Forms.Maps.Map) par défaut, qui centre la carte à Rome :
...
Par défaut, un [`Map`](xref:Xamarin.Forms.Maps.Map) affiche une carte postale si la [`MapType`](xref:Xamarin.Forms.Maps.Map.MapType) propriété n’est pas définie. La `MapType` propriété peut également être définie sur l’un des membres de [`MapType`](xref:Xamarin.Forms.Maps.MapType) l’énumération :
...
Les captures d’écran suivantes [`Map`](xref:Xamarin.Forms.Maps.Map) montrent une [`MapType`](xref:Xamarin.Forms.Maps.Map.MapType) lorsque la propriété a `Street`la valeur :
...
Les captures d’écran suivantes [`Map`](xref:Xamarin.Forms.Maps.Map) montrent une [`MapType`](xref:Xamarin.Forms.Maps.Map.MapType) lorsque la propriété a `Satellite`la valeur :
...
Les captures d’écran suivantes [`Map`](xref:Xamarin.Forms.Maps.Map) montrent une [`MapType`](xref:Xamarin.Forms.Maps.Map.MapType) lorsque la propriété a `Hybrid`la valeur :
...
Cet exemple crée un [`Map`](xref:Xamarin.Forms.Maps.Map) objet qui affiche la zone spécifiée par l' [`MapSpan`](xref:Xamarin.Forms.Maps.MapSpan) objet. L' `MapSpan` objet est centré sur la latitude et la longitude représentées par un [`Position`](xref:Xamarin.Forms.Maps.Position) objet, et s’étend sur les degrés de longitude 0,01 de latitude et 0,01. Pour plus d’informations [`Position`](xref:Xamarin.Forms.Maps.Position) sur la structure, consultez [position et distance](position-distance.md)de la carte. Pour plus d’informations sur le passage d’arguments en XAML, consultez [passage d’arguments en XAML](~/xamarin-forms/xaml/passing-arguments.md).
...
Il existe un certain nombre d’approches pour [`MapSpan`](xref:Xamarin.Forms.Maps.MapSpan) la création d’objets. Une approche courante consiste à fournir les arguments requis au `MapSpan` constructeur. Il s’agit d’une latitude et d’une [`Position`](xref:Xamarin.Forms.Maps.Position) longitude représentées par un objet, et `double` de valeurs qui représentent les degrés de latitude et de `MapSpan`longitude qui sont répartis par le. Pour plus d’informations [`Position`](xref:Xamarin.Forms.Maps.Position) sur la structure, consultez [position et distance](position-distance.md)de la carte.

Il existe également trois méthodes dans la [`MapSpan`](xref:Xamarin.Forms.Maps.MapSpan) classe qui retournent de `MapSpan` nouveaux objets :

1. [`ClampLatitude`](xref:Xamarin.Forms.Maps.MapSpan.ClampLatitude*)retourne un `MapSpan` avec le même `LongitudeDegrees` que l’instance de la classe de la méthode, et un RADIUS `north` défini `south` par ses arguments et.
1. [`FromCenterAndRadius`](xref:Xamarin.Forms.Maps.MapSpan.FromCenterAndRadius*)retourne un `MapSpan` défini par ses [`Position`](xref:Xamarin.Forms.Maps.Position) arguments et [`Distance`](xref:Xamarin.Forms.Maps.Distance) .
1. [`WithZoom`](xref:Xamarin.Forms.Maps.MapSpan.WithZoom*)retourne un `MapSpan` avec le même centre que l’instance de la classe de la méthode, mais avec un rayon multiplié `double` par son argument.

Pour plus d’informations [`Distance`](xref:Xamarin.Forms.Maps.Distance) sur la structure, consultez [position et distance](position-distance.md)de la carte.

Une fois [`MapSpan`](xref:Xamarin.Forms.Maps.MapSpan) qu’un a été créé, les propriétés suivantes sont accessibles pour récupérer des données :
...
Le niveau de zoom d' [`Map`](xref:Xamarin.Forms.Maps.Map) un peut être modifié sans altérer son emplacement. Pour ce faire, vous pouvez utiliser l’interface utilisateur du mappage ou par [`MoveToRegion`](xref:Xamarin.Forms.Maps.Map.MoveToRegion*) programmation en appelant [`MapSpan`](xref:Xamarin.Forms.Maps.MapSpan) la méthode avec un argument qui utilise l' [`Position`](xref:Xamarin.Forms.Maps.Position) emplacement actuel comme argument :
...
Dans cet exemple, la [`MoveToRegion`](xref:Xamarin.Forms.Maps.Map.MoveToRegion*) méthode est appelée avec un [`MapSpan`](xref:Xamarin.Forms.Maps.MapSpan) argument qui spécifie l’emplacement actuel de la carte, via [`Map.VisibleRegion`](xref:Xamarin.Forms.Maps.Map.VisibleRegion) la propriété, et le niveau de zoom en degrés de latitude et de longitude. Le résultat global est que le niveau de zoom de la carte est modifié, mais que son emplacement ne l’est pas. Une autre approche pour implémenter le zoom sur une carte consiste à [`MapSpan.WithZoom`](xref:Xamarin.Forms.Maps.MapSpan.WithZoom*) utiliser la méthode pour contrôler le facteur de zoom.

> [!IMPORTANT]
> Le zoom d’une carte, qu’il s’agisse d’une interface utilisateur de carte [`Map.HasZoomEnabled`](xref:Xamarin.Forms.Maps.Map.HasZoomEnabled) ou d' `true`un programme, requiert que la propriété soit. Pour plus d’informations sur cette propriété, consultez [Disable zoom](#disable-zoom).
...
> [!NOTE]
> La personnalisation du comportement de mappage supplémentaire peut être obtenue en créant un convertisseur personnalisé de carte. Pour plus d’informations, consultez [Personnalisation d’une carte Xamarin. Forms](~/xamarin-forms/app-fundamentals/custom-renderer/map-pin.md).

### <a name="disable-scroll"></a>Désactiver le défilement

La [`Map`](xref:Xamarin.Forms.Maps.Map) classe définit une [`HasScrollEnabled`](xref:Xamarin.Forms.Maps.Map.HasScrollEnabled) propriété de type `bool`. Par défaut, cette propriété `true`a la valeur, ce qui indique que la carte est autorisée à faire défiler. Quand cette propriété a la valeur `false`, le mappage ne défile pas. L’exemple suivant illustre la définition de cette propriété :
...
Dans cet exemple, le `OnMapClicked` gestionnaire d’événements génère la latitude et la longitude qui représentent l’emplacement du mappage taraudé. Le gestionnaire d’événements peut être inscrit avec `MapClicked` l’événement comme suit :
...
- [Personnalisation d’une carte Xamarin.Forms](~/xamarin-forms/app-fundamentals/custom-renderer/map-pin.md)
