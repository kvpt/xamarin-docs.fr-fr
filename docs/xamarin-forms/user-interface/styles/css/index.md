...
> [!NOTE]
> Le sélecteur de `element element` ne requiert pas que l’élément enfant soit un enfant _direct_ du parent : l’élément enfant peut avoir un autre parent. La sélection se produit si un ancêtre est le premier élément spécifié.
...
|`element,element`|`label,button`|Sélectionne tous les éléments de `Button` et tous les éléments de `Label`. Notez que ce sélecteur ne respecte pas la casse.|
|`element element`|`stacklayout label`|Sélectionne tous les éléments `Label` à l’intérieur d’une `StackLayout`. Notez que ce sélecteur ne respecte pas la casse.|
|`element>element`|`stacklayout>label`|Sélectionne tous les éléments `Label` avec `StackLayout` en tant que parent direct. Notez que ce sélecteur ne respecte pas la casse.|
|`element+element`|`label+entry`|Sélectionne tous les éléments `Entry`s directement après une `Label`. Notez que ce sélecteur ne respecte pas la casse.|
|`element~element`|`label~entry`|Sélectionne tous les éléments `Entry` précédés d’un `Label`. Notez que ce sélecteur ne respecte pas la casse.|
...
|Property|S'applique à|Valeurs|Exemple|
|---|---|---|---|
...
|`text-align`| `Entry`, `EntryCell`, `Label`, `SearchBar`|`left` \| `top` \| `right` \| `bottom` \| `start` \| 0 1 2 3 4 5 6. `left` et `right` doivent être évités dans les environnements de droite à gauche.| `text-align: right;`|
