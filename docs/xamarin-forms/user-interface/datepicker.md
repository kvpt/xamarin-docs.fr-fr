...
Si la propriété `BindingContext` de `DatePicker` est définie sur une instance d’un ViewModel contenant des propriétés de type `DateTime` nommées `MinDate`, `MaxDate`et `SelectedDate` (par exemple), vous pouvez instancier le `DatePicker` comme suit :
...
Lorsque l’exemple est tout d’abord exécuté, les deux `DatePicker` vues sont initialisés à la date d’aujourd'hui. La capture d’écran suivante montre le programme s’exécutant sur iOS et Android :
...
- [Exemple de DaysBetweenDates](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-datepicker)
