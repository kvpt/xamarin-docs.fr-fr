...
Sur iOS, cela doit se produire dans **AppDelegate.cs** en appelant la `FormsMaterial.Init` méthode *après* la `Xamarin.Forms.Forms.Init` méthode:
...
Sur Android, cela doit se produire dans **MainActivity.cs** en appelant la `FormsMaterial.Init` méthode *après* la `Xamarin.Forms.Forms.Init` méthode:
