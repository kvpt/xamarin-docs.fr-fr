...
Ce test unitaire vérifie que la validation s’effectue correctement lorsque les deux `ValidatableObject<T>` Propriétés de l' `MockViewModel` instance contiennent des données.
