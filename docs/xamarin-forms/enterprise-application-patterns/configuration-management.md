...
Les paramètres peuvent être lus et écrits via `ISettings` l’API, qui est fournie par la bibliothèque Xam. plugins. Settings. Cette bibliothèque fournit un singleton qui peut être utilisé pour accéder à l’API `CrossSettings.Current`,, et la classe de paramètres d’une application doit exposer ce `ISettings` singleton par le biais d’une propriété.
...
Plutôt que de définir une valeur par défaut `Settings` à l’intérieur `UrlBaseDefault` de la classe, la chaîne obtient `GlobalSetting` sa valeur à partir de la classe. L’exemple de code suivant montre `BaseEndpoint` la propriété `UpdateEndpoint` et la méthode de cette classe :
...
La liaison de données peut être utilisée pour récupérer et définir les paramètres `Settings` exposés par la classe. Cela est réalisé par les contrôles sur la liaison d’affichage pour afficher les propriétés du modèle qui, à `Settings` leur tour, accèdent aux propriétés de la classe, et le déclenchement d’une notification de modification de propriété si la valeur des paramètres a changé. Pour plus d’informations sur la façon dont l’application mobile eShopOnContainers construit les modèles de vue et les associe aux vues, consultez [création automatique d’un modèle de vue avec un localisateur de modèle de vue](~/xamarin-forms/enterprise-application-patterns/mvvm.md#automatically_creating_a_view_model_with_a_view_model_locator).

L’exemple [`Entry`](xref:Xamarin.Forms.Entry) `SettingsView` de code suivant affiche le contrôle de qui permet à l’utilisateur d’entrer une URL de point de terminaison de base pour les microservices en conteneur :

```xaml
<Entry Text="{Binding Endpoint, Mode=TwoWay}" />
```

Ce [`Entry`](xref:Xamarin.Forms.Entry) contrôle crée une liaison avec `Endpoint` la propriété de `SettingsViewModel` la classe, à l’aide d’une liaison bidirectionnelle. L’exemple de code suivant montre la propriété de point de terminaison :
...
Quand accède à, la `InitializeAsync` méthode dans la `SettingsViewModel` classe est exécutée. `SettingsView` L’exemple de code suivant illustre cette méthode :
...
La méthode affecte à `Endpoint` la propriété la valeur de la `UrlBase` propriété dans la `Settings` classe. L’accès à `UrlBase` la propriété entraîne la récupération par la bibliothèque Xam. plugins. Settings de la valeur des paramètres à partir d’un stockage spécifique à la plateforme. Pour plus d’informations sur `InitializeAsync` l’appel de la méthode, consultez [passage de paramètres pendant la navigation](~/xamarin-forms/enterprise-application-patterns/navigation.md#passing_parameters_during_navigation).
