...
1. [Accédez aux données dans Xamarin. Forms](#access-data-in-xamarinforms).
...
Le fichier constants spécifie les valeurs par défaut `SQLiteOpenFlag` énumération utilisées pour initialiser la connexion à la base de données. L’énumération `SQLiteOpenFlag` prend en charge les valeurs suivantes :
...
La méthode `InitializeAsync` est chargée de vérifier si une table existe déjà pour le stockage des objets `TodoItem`. Cette méthode crée automatiquement la table si elle n’existe pas.
...
- L’utilisation de la méthode `Wait` bloque le thread _et_ absorbe les exceptions.

Pour démarrer l’initialisation asynchrone, éviter le blocage de l’exécution et avoir la possibilité d’intercepter des exceptions, l’exemple d’application utilise une méthode d’extension appelée `SafeFireAndForget`. La méthode d’extension `SafeFireAndForget` fournit des fonctionnalités supplémentaires à la classe `Task` :
...
La méthode `SafeFireAndForget` attend l’exécution asynchrone de l’objet `Task` fourni et vous permet d’attacher un `Action` qui est appelé si une exception est levée.
...
La classe Xamarin. Forms `App` expose une instance de la classe `TodoItemDatabase` :
...
Cette propriété permet aux composants Xamarin. Forms d’appeler les méthodes de récupération et de récupération des données sur l’instance `Database` en réponse à l’interaction de l’utilisateur. Par exemple :
