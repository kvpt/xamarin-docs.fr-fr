---
title: Résumé du chapitre 3. Approfondir le texte
description: 'Création d’Mobile Apps avec Xamarin. Forms : Résumé du chapitre 3. Approfondir le texte'
ms.prod: xamarin
...
---
# <a name="summary-of-chapter-3-deeper-into-text"></a>Résumé du chapitre 3. Approfondir le texte
...
Une fois créée, une valeur de `Color` est immuable. Les caractéristiques de la couleur peuvent être obtenues à partir des propriétés suivantes :
...
`Color` définit également les champs statiques publics 240 en lecture seule pour les couleurs courantes. Au moment de la rédaction du livre, seules 17 couleurs courantes étaient disponibles.
...
`Color.Default` est destiné à appliquer le modèle de couleurs de la plateforme, et a donc une signification différente dans différents contextes sur différentes plateformes. Par défaut, les modèles de couleurs de la plateforme sont les suivants :
...
Affectez à la propriété [`FontFamily`](xref:Xamarin.Forms.Label.FontFamily) de `Label` une chaîne telle que « Times Roman » pour sélectionner une famille de polices. Toutefois, vous devez spécifier une famille de polices qui est prise en charge sur la plateforme particulière et les plateformes ne sont pas cohérentes à cet égard.
...
- [Ajouter des contrôles](~/xamarin-forms/user-interface/text/label.md)
