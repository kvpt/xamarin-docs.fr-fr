...
Cette classe définit une `CustomPin` comme héritant des propriétés de la classe [`Pin`](xref:Xamarin.Forms.Maps.Pin) , et ajoute des propriétés `Name` et `Url`.
