...
Par défaut, une petite animation de navigation se produit lorsqu’une page est parcourue à l’aide de la méthode `GoToAsync`. Toutefois, ce comportement peut être modifié en affectant à la `Shell.PresentationMode` propriété jointe sur un [`ContentPage`](xref:Xamarin.Forms.ContentPage) l’un des membres de l’énumération `PresentationMode` :

- `NotAnimated` indique que la page s’affiche sans animation de navigation.
- `Animated` indique que la page sera affichée avec une animation de navigation. Il s’agit de la valeur par défaut de la propriété jointe `Shell.PresentationMode`.
...
Dans cet exemple, la [`ContentPage`](xref:Xamarin.Forms.ContentPage) est définie pour s’afficher en tant que page modale, lorsque la page est parcourue avec la méthode `GoToAsync`.

## <a name="enable-navigation-bar-shadow"></a>Activer l’ombre de la barre de navigation

La classe `Shell` définit la `NavBarHasShadow` propriété jointe, de type `bool`, qui contrôle si l’ombre de la barre de navigation est. Par défaut, la valeur de la propriété est `false` sur iOS et `true` sur Android.
...
L’interpréteur de commandes respecte la visibilité de la page, définie avec la propriété [`IsVisible`](xref:Xamarin.Forms.VisualElement.IsVisible) . Par conséquent, lorsque la propriété `IsVisible` d’une page est définie sur `false` elle n’est pas visible dans l’application Shell et il n’est pas possible d’y accéder.
