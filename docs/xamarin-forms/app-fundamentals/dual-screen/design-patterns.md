---
title: Modèles de conception Xamarin.Forms pour appareils double écran
description: Ce guide explique comment Xamarin.Forms prend en charge un large éventail de modèles de conception optimisés pour les appareils double écran.
ms.prod: xamarin
...
Le modèle maître/détail désigne une configuration dans laquelle un utilisateur sélectionne un élément de contenu dans une vue maître, en général une liste côté gauche, pour obtenir des détails sur cet élément côté droit.
...
Le [`CollectionView`](xref:Xamarin.Forms.CollectionView) avec une disposition en grille scindée le long de la charnière est une méthode idéale pour offrir cette expérience double écran.

## <a name="dual-view-pattern"></a>Modèle d’affichage double

L’affichage double peut ressembler à la vue « à deux pages », mais la distinction se situe dans le contenu et le scénario utilisateur. Dans ce modèle, vous comparez le contenu côte à côte, par exemple pour modifier un document ou une photo, comparer des menus de restaurant ou trouver les différences dans un conflit de fusion pour des fichiers de code.
...
- [Exemples pour appareils double écran (GitHub)](https://github.com/xamarin/xamarin-forms-samples/tree/pre-release/UserInterface/DualScreenDemos)
