...
description: Visual Studio pour Mac peut être utilisé pour créer et intégrer du code multiplateforme C/C++ dans les applications mobiles pour Android et iOS, à l’aide de Xamarin et de C#. Cet article explique comment installer et déboguer un projet C++ dans une application Xamarin.
...
ms.locfileid: "92928501"
---
# <a name="use-cc-libraries-with-xamarin"></a>Utiliser des bibliothèques C/C++ avec Xamarin

## <a name="overview"></a>Vue d’ensemble

Xamarin permet aux développeurs de créer des applications mobiles natives multiplateformes avec Visual Studio. En règle générale, les liaisons C# sont utilisées pour exposer des composants de plateforme existants aux développeurs. Toutefois, il existe des cas où les applications Xamarin doivent fonctionner avec les codes base existants. Parfois, les équipes n’ont pas le temps, le budget ou les ressources nécessaires pour porter un code base volumineux, bien testé et hautement optimisé vers C#.

[Visual C++ pour le développement mobile multiplateforme](/visualstudio/cross-platform/visual-cpp-for-cross-platform-mobile-development) permet de générer le code C/C++ et C# dans le cadre de la même solution, en offrant de nombreux avantages, notamment une expérience de débogage unifiée. Microsoft a utilisé C/C++ et Xamarin de cette façon pour fournir des applications telles que l' [appareil photo](https://www.microsoft.com/microsoftpix) [mobile](https://www.microsoft.com/p/hyperlapse-mobile/9wzdncrd1prw) et pix.

Toutefois, dans certains cas, il existe un désir (ou une exigence) de conserver les outils et les processus C/C++ existants et de conserver le code de bibliothèque dissocié de l’application, en traitant la bibliothèque comme si elle était similaire à un composant tiers. Dans ce cas, le défi consiste à exposer les membres pertinents à C#, mais à gérer la bibliothèque en tant que dépendance. Et, bien sûr, en automatisant autant que possible le processus.  
...
L’illustration ci-dessous représente l’approche à quatre étapes utilisée pour transformer le code source C/C++ en une bibliothèque Xamarin multiplateforme partagée via NuGet, puis utilisée dans une application Xamarin. Forms.
...
### <a name="stage-1-compiling-the-cc-source-code-into-platform-specific-native-libraries"></a>Étape 1 : compilation du code source C/C++ dans des bibliothèques natives spécifiques à une plateforme

L’objectif de cette étape est de créer des bibliothèques natives qui peuvent être appelées par le wrapper C#. Cela peut être utile en fonction de votre situation. Les nombreux outils et processus qu’il est possible d’introduire dans ce scénario courant n’entrent pas dans le cadre de cet article. Les principales considérations sont le maintien de la synchronisation du code base C/C++ avec tout code wrapper natif, un test unitaire suffisant et l’automatisation de la génération.
...
- [Ligne de commande NuGet (CLI)](/nuget/tools/nuget-exe-cli-reference#macoslinux)
...
La fonctionnalité de la bibliothèque native est basée sur l’exemple de [procédure pas à pas : création et utilisation d’une bibliothèque statique (C++)](/cpp/windows/walkthrough-creating-and-using-a-static-library-cpp?view=vs-2017).
...
Une classe supplémentaire définit des fonctions wrapper qui permettent à un consommateur .NET de créer, de supprimer et d’interagir avec la classe Native sous-jacente `MyMathFuncs` .
...
4. Configurez les propriétés de la **référence Native** afin qu’elles soient activées (avec une icône représentant une coche) dans le panneau **Propriétés** :
    ...
    > [!NOTE]
    > L’utilisation d’un type de projet de bibliothèque de liaison avec une [référence Native](../macios/native-references.md) incorpore la bibliothèque statique et lui permet d’être liée automatiquement à l’application Xamarin. IOS qui la référence (même si elle est incluse via un package NuGet).

5. Ouvrez **ApiDefinition.cs** , en supprimant le code commenté par un modèle (en laissant uniquement l' `MathFuncs` espace de noms), puis effectuez la même étape pour **structs.cs**
...
À présent, écrivez le code C# pour appeler la bibliothèque native. L’objectif est de masquer toute complexité sous-jacente. Le consommateur ne doit pas avoir de connaissances pratiques sur les éléments internes de la bibliothèque native ou sur les concepts P/Invoke.  
...
3. Implémentez la classe **MyMathFuncsSafeHandle** :
    ...
    > [!NOTE]
    > Un [SafeHandle](/dotnet/api/system.runtime.interopservices.safehandle?view=netframework-4.7.2) est la meilleure façon de travailler avec des ressources non managées dans du code managé. Cela élimine un grand nombre de code réutilisable lié à la finalisation critique et au cycle de vie des objets. Le propriétaire de ce descripteur peut ensuite le traiter comme n’importe quelle autre ressource gérée et n’a pas besoin d’implémenter le [modèle entièrement jetable](/dotnet/standard/garbage-collection/implementing-dispose).
...
Maintenant que le wrapper est terminé, créez une classe MyMathFuncs qui gérera la référence à l’objet MyMathFuncs C++ non managé.  
...
4. Mettez à jour **MathFuncs. NuSpec** avec les métadonnées de package de base à afficher au consommateur **NuGet** . Par exemple :
    ...
    > [!NOTE]
    > Consultez le document de [référence NuSpec](/nuget/reference/nuspec) pour plus d’informations sur le schéma utilisé pour ce manifeste.

5. Ajoutez un `<files>` élément en tant qu’enfant de l' `<package>` élément (juste en dessous `<metadata>` ), en identifiant chaque fichier avec un `<file>` élément distinct :
...
8. Ajoutez les `<file>` éléments pour les assemblys **netstandard 2.0** :
...
3. Dans **Terminal** , exécutez la commande du **Pack NuGet** en spécifiant le fichier **NuSpec** , la **version** (par exemple, 1.0.0) et le **outputDirectory** à l’aide du dossier créé à l' [étape précédente](#preparing-a-local-packages-directory), autrement dit, **local-NuGet-Feed** . Par exemple :
...
Une technique plus robuste est décrite dans [prise en main des packages NuGet dans Azure DevOps](/azure/devops/artifacts/get-started-nuget?tabs=new-nav&view=vsts#publish-a-package), qui montre comment créer un flux privé et transmettre le package (généré à l’étape précédente) à ce flux.

Il est idéal de faire en sorte que ce flux de travail soit entièrement automatisé, par exemple à l’aide de [Azure pipelines](/azure/devops/pipelines/index?view=vsts). Pour plus d’informations, consultez la page [prise en main de Azure pipelines](/azure/devops/pipelines/get-started/index?view=vsts).
...
7. Choisissez l’émulateur d' **appareil** ou de **simulateur** préféré / **Emulator** .
...
3. Vérifiez que la **version** du package est **1.0.0** et que les autres détails s’affichent comme prévu, par exemple le **titre** et la **Description** , autrement dit, *MathFuncs* et exemple de *bibliothèque de wrappers C++* .
...
4. Substituez les `OnAppearing` `OnDisappearing` méthodes et de la `ContentPage` classe de base :
...
- [Port d’exemple San Angeles mono](/samples/xamarin/monodroid-samples/sanangeles-ndk/)
